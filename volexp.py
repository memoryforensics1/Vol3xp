...
# VolExp
# Author: Aviel Zohar
# See license information on VolExp github page: https://github.com/memoryforensics1/Vol3xp
...
# Imports
import urllib.request, urllib.parse, urllib.error	# virustotal check.
from webbrowser import open_new                     # go to github page
from hashlib import sha256
import io, string, pickle, re, json, struct
import os, sys, time, logging, random, copy, math
import threading, queue, subprocess, inspect, functools
import turtle, tkinter.ttk
from tkinter.ttk import Frame, Treeview, Scrollbar, Progressbar, Combobox
...
try:
    ...
    has_themes = False
    vollog.log(constants.LOGLEVEL_VVV, "Install ttkthemes for better themes on volexp")
...
try:
    ...
    has_pefile = False
    vollog.log(constants.LOGLEVEL_VVV, "Install pefile for better pe analysis on volexp")

try: #TODO-> change to capstone
    import distorm3
    has_distorm = True
except ImportError:
    has_distorm = False
...
try:
    ...
    has_requests = False
    vollog.log(constants.LOGLEVEL_VVV,  'to run this plugin using virustotal result please install requests module (run on shell "pip install requests")')
...
Main process tab:
    ...
    Process Right Click:
        ...
        Struct Analyzer - Run structanalysis plugin with type _EPROCESS and the address of 
        ...
        Properties [Double Click]- Open properties tab:
            ...
            Threads - Display all the process's threads, 
                ...
                Struct Analyzer - open a new tab with StructAnalyzer plugin.
      ...
      _____\//\\\\\______\//\\\__/\\\_____\/\\\____\/\\\________________/\\\/\\\___\/\\\_________  
       ______\//\\\________\///\\\\\/____/\\\\\\\\\_\/\\\\\\\\\\\\\\\__/\\\/\///\\\_\/\\\_________ 
        _______\///___________\/////_____\/////////__\///////////////__\///____\///__\///__________
                ...
                | | __ / _ \ / _ \ / _` | |   | | | |/ __| |/ / 
                | |_\ \ (_) | (_) | (_| | |___| |_| | (__|   <  
                 \____/\___/ \___/ \__,_|\____/\__,_|\___|_|\_\ 
...
TIP_LIST = ['Tip of the investigation\nAll the explorers have a search [ctrl+f] options\n(File Explorer, WinObj Explorer, MFT explorer, Struct Analyzer)',
            ...
            'Tip of the investigation\nIf you need more information on something you probebly can get this data using Struct Analyzer',
...
def get_func_args(func):
    ...
    class Dummy(object):
        def __init__(self):
            ...
            self.defaults = []
    to_return = Dummy()
    for parm in inspect.signature(func).parameters.items():
        to_return.args.append(parm[1].name)
        to_return.defaults.append(parm[1].default if parm[1].default!=inspect._empty else '')
    return to_return
...
def start_vol(file_path, command):
    '''
    ...
    :return:
    '''
    os.system('start cmd /k echo {} {} -f {} {}'.format(sys.executable, sys.argv[0], file_path, command))

def run_struct_analyze(struct_type, address, app=None, pid=4, as_thread=True, write_sup=True):
    '''
    Run the Struct Analyzer plugin
    ...
    self = volself
    sa_conf = self.context.clone()
    sa_conf.ADDR = address#str(hex(address)).replace('L','') if not str(address).startswith('0x') else address
    sa_conf.STRUCT = struct_type

    # Here to support write (when volatility enable it)
    ...
    # Make sure we use an exists layer name
    if pid in sa_conf.layers._layers or type(pid) is int:
        sa_conf.PID = pid
    elif '_' in str(pid):
    ...
    print('run_struct_analyze',sa_conf.ADDR, sa_conf.STRUCT, sa_conf.PID)
    my_sa = StructAnalyzer(sa_conf, self.config_path)
...
def dump_file(offset, nt, directory=None):
    '''
    ...
    SharedCacheMap =  get_right_member(file_obj, ["SectionObjectPointer.SharedCacheMap"])
    ImageSectionObject = get_right_member(file_obj, ["SectionObjectPointer.ImageSectionObject"])

    def extract_file_from_control_area(control_area):
        """
        This function get control area object and extract the file from this object
        :param ControlArea: ControlArea object
        :return:
        """
        valid = True
        # Validate the segment:
        try:
            if control_area.Segment.ControlArea.dereference().vol.offset != control_area.vol.offset:
                valid = False
            #elif control_area.Segment.SizeOfSegment != control_area.Segment.TotalNumberOfPtes * 0x1000:
            #	valid = False
        ...
        if valid:
            ...
            while subsection not in seen_subs and subsection:
                ...
                while (ptecount < PtesInSubsection):
                    pteoffset = SubsectionBase + (nt.get_type('_MMPTE').size * ptecount)
                    ...
                    try:
                        if mmpte.u.Hard.Valid == 1:
                            physical_offset = mmpte.u.Hard.PageFrameNumber << 12
                            file_data.append([physical_offset, FileOffset, 0x1000])
                        elif mmpte.u.Soft.Prototype == 1 or mmpte.u.Soft.Transition == 0:
                            zero_file_pad.append([FileOffset, 0x1000])
                        elif mmpte.u.Trans.Transition ==1:
                            physical_offset = mmpte.u.Trans.PageFrameNumber << 12
                            file_data.append([physical_offset, FileOffset, 0x1000])
                        else:
                            zero_file_pad.append([FileOffset, 0x1000])
                    except exceptions.InvalidAddressException:
                ...
                try:
                    ...
                    break
        return(file_data, zero_file_data)


    def extract_file_from_cache(shared_cache_map):
        """
        This function extract the file data from shared_cache_map object.
        :param shared_cache_map: _SHARED_CACHE_MAP object
        :return:
        """
    ...
    if DataSectionObject:
        control_area = DataSectionObject.dereference().cast('_CONTROL_AREA')
        ca_offset_string = "0x{0:x}".format(control_area.vol.offset) + '.' + file_name.split("\\")[-1]
        file_string = ".".join(["file", ca_offset_string, 'dat'])
        of_path = os.path.join(directory or self._config.DUMP_DIR, file_string)
        (mdata, zpad) = extract_file_from_control_area(control_area)
        summaryinfo = {}
        summaryinfo['name'] = file_name
        ...
        summaryinfo['pid'] = '0'
        file_data.append(summaryinfo)
    if ImageSectionObject:
        control_area = ImageSectionObject.dereference().cast('_CONTROL_AREA')
        ca_offset_string = "0x{0:x}".format(control_area.vol.offset) + '.' + file_name.split("\\")[-1]
        file_string = ".".join(["file", ca_offset_string, 'img'])
        of_path = os.path.join(directory or self._config.DUMP_DIR, file_string)
        (mdata, zpad) = extract_file_from_control_area(control_area)
        summaryinfo = {}
        summaryinfo['name'] = file_name
        ...
        file_data.append(summaryinfo)
    if SharedCacheMap:
        shared_cache_map = SharedCacheMap.dereference().cast("_SHARED_CACHE_MAP")
        shared_cache_map_string = ".0x{0:x}".format(shared_cache_map.vol.offset) + '.' + file_name.split("\\")[-1]
        of_path = os.path.join(directory or self._config.DUMP_DIR, "file." + shared_cache_map_string + ".vacb")
        vacbary = extract_file_from_cache(shared_cache_map)
        summaryinfo['name'] = file_name
...
def dump_explorer_file(offset, directory=None):
    ...
    def audited_read_bytes(vm, vaddr, length, pad):
        zpad = []
        mdata = []
    ...
    if self._config.DUMP_DIR == None:
        vollog.log(constants.LOGLEVEL_VVV,  "Please specify a dump directory (--dump-dir)")
        return
    ...
    for summaryinfo in df_calc:
        ...
        if summaryinfo['type'] == "DataSectionObject":
            ...
            for mdata in summaryinfo['present']:
                ...
                try:
                    rdata = def_conf.layers['memory_layer'].read(mdata[0], mdata[2])
                except (IOError, OverflowError):
                    vollog.log(constants.LOGLEVEL_VVV, "IOError: Pid: {0} File: {1} PhysAddr: {2} Size: {3}".format(summaryinfo['pid'], summaryinfo['name'], mdata[0], mdata[2]))
            ...
            for vacb in summaryinfo['vacbary']:
                if not vacb:
                    continue
                (rdata, mdata, zpad) = audited_read_bytes(def_conf.kaddr_space, vacb['baseaddr'], vacb['size'], True)
...
def virus_total(hash, process_name, pid, file_path, apikey=None):
    ...
    try:
        ...
        vollog.log(constants.LOGLEVEL_VVV,  'Virus Total Failed: you are not connected to the internet/you disconnected for a momonet(try to see if there is connection problems and try again later).')
        return
    ...
    if int(pid) not in plugins_output:
    ...
    print(write)
    if 'scans' in result:
        for antiVirus in result['scans']:
            av_result = result['scans'][antiVirus]
...
class PropertiesClass(Frame):
    ...
    def __init__(self, master, menu_show='Image', selection=None, relate=None,  *args, **kwargs):
        ...
        self.title_font = tkinter.font.Font(family='Ariel', size=16, weight="bold", slant="italic")
...
class Image(Frame):
    ...
    def explore(self, path):
        ...
        # Validate drive
        if drive not in winobj_dict['/']['GLOBAL??']:
    ...
    def __init__(self, parent, controller):
        ...
        label = tkinter.ttk.Label(self, text="Image", font=controller.title_font)
        ...
        beuty_frame = tkinter.ttk.Frame(self)
        lable_frame = tkinter.ttk.LabelFrame(self, text=str(os.path.splitext(process)[0].title() if process else '???'))  # controller.title_font)
        lable_frame.pack(padx=5, pady=5, anchor="w")
        left_frame = tkinter.ttk.Frame(lable_frame)
        ...
        txt_width = tkinter.font.Font().measure(self.path) // 7
        txt_width = txt_width if txt_width > tkinter.font.Font().measure(self.current_directory) else tkinter.font.Font().measure(self.current_directory)
        ...
        tkinter.ttk.Label(left_frame, text='').pack(anchor="w")

        # Create path and button frames
        lf = tkinter.ttk.Frame(left_frame)
        rf = tkinter.ttk.Frame(left_frame)

        # Path button and text widget
        path_button = tkinter.ttk.Button(lf, text="Path:", command=self.path_click)
        ToolTip(path_button, "Open In Explorer")
        path_button.pack(fill=tk.BOTH)
        txt_entry = tkinter.ttk.Entry(rf, width=txt_width)
        ...
        dir_button = tkinter.ttk.Button(lf, text="Current Directory:", command=self.current_directory_click)
        ...
        txt_entry.state(['readonly'])
        txt_entry.pack(anchor="w", ipady=1, pady=1)
        tkinter.ttk.Label(left_frame, text='\n').pack(anchor="w")
        ...
        left_frame2 = tkinter.ttk.Frame(lable_frame)
        tlf = tkinter.ttk.Frame(left_frame2)
        trf = tkinter.ttk.Frame(left_frame2)
        ...
        for my_string in my_strings:
            ...
            txt_entry = tkinter.ttk.Entry(trf, width=(txt_width if txt_width < 250 else 250))
        ...
        if additional_info != '':
            label_frame = tkinter.ttk.LabelFrame(beuty_frame, text='Additional Information')
            label_frame.pack(padx=5, pady=5, anchor="w")
            _frame = tkinter.ttk.Frame(label_frame)
            for c_string in additional_info.splitlines():
                my_label = tkinter.ttk.Label(_frame, text=str(c_string))
                my_label.pack(anchor="w")
...
class Imports(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        Frame.__init__(self, parent)
        self.controller = controller
        label = tkinter.ttk.Label(self, text="Imports", font=controller.title_font)
        label.config(anchor="center")
        label.pack(side="top", fill="x", pady=10)
        data = []
        ...
        if int(self.controller.pid) in process_imports:
...
class Performance(Frame):
    ...
    def __init__(self, parent, controller):
        Frame.__init__(self, parent)
        self.controller = controller
        label = tkinter.ttk.Label(self, text="Performance", font=controller.title_font)
        label.config(anchor="center")
        label.pack(side="top", fill="x", pady=10)
        ...
        left_frame = tkinter.ttk.Frame(self)
        right_frame = tkinter.ttk.Frame(self)
        frame_size = 20


        # CPU
        label_frame = tkinter.ttk.LabelFrame(left_frame, text='CPU')
        label_frame.pack(fill=tk.BOTH, padx=7, pady=7)
        my_frame = tkinter.ttk.Frame(label_frame)
        cpu_info = '\n'.join(('Priority:', 'Kernel Time:', 'User Time:', 'Total Time:', 'Cycles:'))
        cpu_label = tkinter.ttk.Label(my_frame, text=cpu_info, wraplength=500)
        ...
        # Virtual Memory
        label_frame = tkinter.ttk.LabelFrame(left_frame, text='Virtual Memory')
        label_frame.pack(fill=tk.BOTH, padx=5, pady=5)
        my_frame = tkinter.ttk.Frame(label_frame)
        vm_info = '\n'.join(('Private Bytes:', 'Peak Private Bytes:', 'Virtual:', 'Page Faults:'))
        vm_label = tkinter.ttk.Label(my_frame, text=vm_info, wraplength=500)
        ...
        # Physical Memory
        label_frame = tkinter.ttk.LabelFrame(left_frame, text='Physical Memory')
        label_frame.pack(fill=tk.BOTH, padx=5, pady=5)
        my_frame = tkinter.ttk.Frame(label_frame)
        pm_info = '\n'.join(('Memory Priority:', 'Working Set:', 'WS Private:', 'WS:', 'WS Shared:', 'Peak Working Set:'))
        pm_label = tkinter.ttk.Label(my_frame, text=pm_info, wraplength=500)
        ...
        # I/O
        label_frame = tkinter.ttk.LabelFrame(right_frame, text='I/O')
        label_frame.pack(fill=tk.BOTH, padx=5, pady=5)
        my_frame = tkinter.ttk.Frame(label_frame)
        ...
        io_label = tkinter.ttk.Label(my_frame, text=io_info, wraplength=500)
        ...
        # Handles
        label_frame = tkinter.ttk.LabelFrame(right_frame, text='Handles')
        label_frame.pack(fill=tk.BOTH, padx=5, pady=5)
        my_frame = tkinter.ttk.Frame(label_frame)
        handles_info = '\n'.join(('Handles:', 'Peak Handles:', 'GDI Handles:', 'USER Handles:'))
        handle_label = tkinter.ttk.Label(my_frame, text=handles_info, wraplength=500)
...
class Services(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        Frame.__init__(self, parent)
        self.controller = controller
        label = tkinter.ttk.Label(self, text="Services", font=controller.title_font)
        label.config(anchor="center")
        label.pack(side="top", fill="x", pady=10)
        data = []

        # Get the data for the table (if there is any).
        if self.controller.pid in service_dict:
...
class Threads(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        if not main_table.tree.item(item)['values'][-1]:
            label = tkinter.ttk.Label(self, text="Unable to find _EPROCESS struct of this process..", font=controller.title_font)
            label.config(anchor="center")
            label.pack(side="top", fill="x", pady=10)
            return

        label = tkinter.ttk.Label(self, text="Threads", font=controller.title_font)
        label.config(anchor="center")
        label.pack(side="top", fill="x", pady=10)
        data = []
        ...
        if int(self.controller.pid) in process_threads:
            data = list(process_threads[int(self.controller.pid)].values())
        ...
        self.thread_treetable.aMenu.add_command(label='Struct Analyzer ', command=self.run_struct_analyze)
    ...
    def viewHexDump(self, event):
        ...
        with lock:
            ...
            hex_dis = volself.disassemble(self.proc._context.layers[self.proc.add_process_layer()], start_addr if type(start_addr) is not str else (start_addr.strip('L')))
...
class TcpIp(Frame):
    ...
    def __init__(self, parent, controller):
        Frame.__init__(self, parent)
        self.controller = controller
        label = tkinter.ttk.Label(self, text="TcpIp", font=controller.title_font)
        label.config(anchor="center")
        label.pack(side="top", fill="x", pady=10)

        # Get the connections data
        data = [ tup for tup in process_connections[self.controller.pid]] if self.controller.pid in process_connections else [("There", "is", "no", "conenctions", "at", "all") if len(process_connections) > 0 else ["searching", "for", "connections,", "try", "again", "later"]]
...
class Security(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # Get the user sid.
        if 'Groups' in process_security[self.controller.pid]:
        ...
        if 'Groups' in process_security[pid]:
        ...
        if 'Privs' in process_security[pid]:
...
class Environment(Frame):
    ...
    def __init__(self, parent, controller):
        Frame.__init__(self, parent)
        self.controller = controller
        label = tkinter.ttk.Label(self, text="Environment", font=controller.title_font)
        label.config(anchor="center")
        label.pack(side="top", fill="x", pady=10)

        # Get the environment data
        data = [(tup, process_env_var[self.controller.pid][tup]) for tup in process_env_var[self.controller.pid]] if self.controller.pid in process_env_var else []
...
class Job(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        if not main_table.tree.item(item)['values'][-1]:
            ...
            label.pack(side="top", fill="x", pady=10)
            return
        label = tkinter.ttk.Label(self, text="Job", font=controller.title_font)
        label.config(anchor="center")
        label.pack(side="top", fill="x", pady=10)
        ...
        if job:
            for task in job.ProcessListHead.to_list('{}{}_EPROCESS'.format(e_proc.get_symbol_table_name(), constants.BANG), 'JobLinks'):
                try:
                    data.append((task.get_peb().ProcessParameters.ImagePathName.String, task.UniqueProcessId, task.InheritedFromUniqueProcessId))
                except:
                    data.append((objects.utility.array_to_string(task.ImageFileName), task.UniqueProcessId, task.InheritedFromUniqueProcessId))
...
class Plugin(Frame):
    ...
    def __init__(self, parent, controller, plugin_tup):
        ...
        self.controller = controller
        item = self.controller.selection
        label = tkinter.ttk.Label(self, text=plugin_tup[0].title(), font=controller.title_font)
        label.config(anchor="center")
        label.pack(side="top", fill="x", pady=10)
        data = []
...
class PEImage(Frame):
    ...
    def apply_all_process(self, event=None):
        ...
        if ans == None:
            ...
            for pid in pe_comments['pid']:
                if str(self.path) in pe_comments['pid'][pid]:
    ...
    def __init__(self, parent, controller):
        ...
        self.path = path = self.parent.my_dll.FullDllName.String
        ...
        # Path button and text widget
        path_frame = tkinter.ttk.Frame(left_frame)
        path_button = tkinter.ttk.Button(path_frame, text="Path:\t\t\t", command=self.path_click)
        ToolTip(path_button, "Open In Explorer")
        path_button.pack(side=LEFT)
        self.path = str(path)
        #txt_width = tkFont.Font().measure(self.path) // 7
        txt_entry = tkinter.ttk.Entry(path_frame, width=txt_width)
        txt_entry.insert(0, self.path)
        txt_entry.state(['readonly'])
        ...
        for my_string in my_strings:
            ...
            my_label = tkinter.ttk.Label(lf, text=label_txt, wraplength=500)
            my_label.pack(anchor="w")
            txt_entry = tkinter.ttk.Entry(rf, width=txt_width)
            txt_entry.insert(0, entry_txt)
            txt_entry.state(['readonly'])
        ...
        for c_mod in self.parent.nt_header.get_sections():
            ...
            data.append((objects.utility.array_to_string(c_mod.Name), c_mod.Misc, c_mod.VirtualAddress, c_mod.SizeOfRawData, c_mod.PointerToRawData, c_mod.PointerToRelocations, c_mod.PointerToLinenumbers, c_mod.NumberOfRelocations, c_mod.NumberOfLinenumbers, c_mod.Characteristics, sh_characteristics_str))
        ...
        save_button = tkinter.ttk.Button(self, text="Apply Comment on all processes", command=self.apply_all_process)
        save_button.pack()
...
class RegViewer(Frame):
    ...
    def __init__(self, master, dict, headers, reg_api, *args, **kwargs):
        Frame.__init__(self, master, *args, **kwargs)
        self.back_button = tkinter.ttk.Button(self, text="\t Registry")
        self.back_button.pack(fill='x')
        self.panedWindow = tkinter.ttk.Panedwindow(self, orient=tk.HORIZONTAL)  # orient panes horizontally next to each other
        self.panedWindow.pack(fill=tk.BOTH, expand=True)                # occupy full master window and enable expand property

        self.frame1 = tkinter.ttk.Frame(self.panedWindow, width=200, height=300, relief=tk.SUNKEN)
        self.frame2 = tkinter.ttk.Frame(self.panedWindow, width=400, height=400, relief=tk.SUNKEN)
        ...
        self.r1 = tkinter.ttk.Radiobutton(self.frame2, text='View Data (Slow, Searching).', width=20, variable=self.var, value=True,
                             command=self.viewKeys,
                             style='IndicatorOff.TRadiobutton')
        self.r1.pack(fill='x')
        self.r2 = tkinter.ttk.Radiobutton(self.frame2, text="Don\'t View Data (Fast).", width=20, variable=self.var, value=False,
                             command=self.viewKeys,
                             style='IndicatorOff.TRadiobutton')
        self.r2.pack(fill='x')
        ...
        self.dict = dict
        data = []
        hives_list = list(self.dict.keys())
        ...
        for key in hives_list:
            ...
            if key != "|properties|":
                ...
                if "|properties|" in self.dict[key]:
        ...
        self.regapi = {}
        for c_hive in reg_api:
            self.regapi[c_hive.get_name().lower().split('\\')[-1]] = c_hive
        self.tree = self.keys
        self.row_search = ('', 0)
    ...
    def __init_items__(self, folder, count=0, maxim=2):
        ...
        data = []
        for key in list(pMftExp.keys()):
            if key != "|properties|":
                my_tup = tuple([0 for i in range(len(self.headers)-1)])
                if "|properties|" in pMftExp[key]:
    ...
    def show_table_thread(self, full_path):
        ...
        try:
            key = self.regapi[hive_name.lower()].get_key(full_path)
        except exceptions.InvalidAddressException:
            print("[-] Failed on key parsing (InvalidAddressException)")
        ...
        for v in key.get_values():
            ...
            try:
                ...
                vollog.debug(excp)
                value_data = 'UNKNOWN'#renderers.UnreadableValue()''

            #try:
            #	value_data = value_data.decode().replace('\x00', '')
            #except:
            #	pass

            value, typ, data_info, addr = v.get_name(), typ, value_data, v
            ...
            elif typ in ('REG_FULL_RESOURCE_DESCRIPTOR', 'REG_RESOURCE_LIST',): #'REG_BINARY'
                data.append((str(value), typ, text_renderer.multitypedata_as_text(format_hints.MultiTypeData(data_info.encode("utf-16-le"),encoding='utf-16-le',split_nulls=True)), addr.vol.offset))
            elif type(data_info) is list:
                data.append((str(value), typ, data_info, addr.vol.offset))
            else:
                data.append((str(value), typ, data_info, addr.vol.offset))
                ...
                for key in dict:
                    ...
                    if ans:
                        try:
                            ...
                            print('[-] Unicode error at: {}\nThis object will exclude from the csv.'.format(uniError))
...
class FileExplorer(Frame):
    ...
    def __init__(self, master, dict, headers, searchTitle, resize=True, path=None, relate=None,  *args, **kwargs):
        ...
        self.pw = tkinter.ttk.PanedWindow(self, orient=tk.HORIZONTAL)
        self.exp_frame = Explorer(self.pw, dict[' '], headers, searchTitle, resize, path, relate, *args, **kwargs)
        self.tree = self.exp_frame.tree
        self.exp_frame.tree.aMenu.add_command(label='HexDump', command=self.HexDump) # todo-> create dumpfiles for vol3
        self.exp_frame.tree.aMenu.add_command(label='Dump', command=self.Dump)
        self.exp_frame.tree.aMenu.add_command(label='StructAnalyzer', command=self.run_struct_analyze)
        ...
        if dict['?userassist?'] != []:
            ...
            headers = ('Hive Offset', 'Registry', 'Path', 'Last Write', 'Type', 'Value (file path)', 'ID', 'Count', 'Focus Count', 'Time Focused', 'Last Updated', 'Raw Data')
            display_headers = ('Value (file path)', 'ID', 'Count', 'Focus Count', 'Last Write')
            ua_frame.tree = TreeTree(ua_frame, headers=headers, data=dict['?userassist?'], text_by_item=5, resize=False, display=display_headers)
    ...
    def run_struct_analyze(self):
        '''
        get address and send to the Struct Analyzer function.
        '''
        item = self.tree.tree.selection()[0]
        struct_type = "_FILE_OBJECT"
        addr = self.tree.tree.item(item)['values'][-1]
        if not addr:
            print("[-] Unable to find the address of this {}".format(struct_type))
            return
        print("[+] Run Struct Analyzer on {}, addr: {}".format(struct_type, addr))
        threading.Thread(target=run_struct_analyze, args=(struct_type, addr, None, 'physical')).start()
    ...
    def resolve_path_and_go_to(self, tree):
        ...
        for c_pid in process_bases:
            if 'wininit.exe' in process_bases[int(c_pid)]["dlls"]:
                if int(c_pid) in process_env_var:
                    for env in process_env_var[int(c_pid)]:
                        path = path.replace('%{}%'.format(env.lower()), process_env_var[int(c_pid)][env])

        for c_pid in process_bases:
            if 'explorer.exe' in [c_exp.lower() for c_exp in list(process_bases[int(c_pid)]["dlls"].keys())]:
                if int(c_pid) in process_env_var:
                    for env in process_env_var[int(c_pid)]:
                        path = path.replace('%{}%'.format(env.lower()), process_env_var[int(c_pid)][env])
        ...
        if winobj_dict == {}:
            ...
            return
        elif drive not in winobj_dict['/']['GLOBAL??']:
            messagebox.showerror('Error', 'Invalid Drive: {}'.format(drive), parent=self)
            return
        device = winobj_dict['/']['GLOBAL??'][drive]['|properties|'][-2].replace('Target: ', '')
    ...
    def dump_dir(self, pMftExp, current_path, recursive=True):
        ...
        for key in list(pMftExp.keys()):
            ...
            if type(pMftExp[key]) is dict and len(pMftExp[key]) - ("|properties|" in pMftExp[key]) > 0:
                ...
                try:
                    ...
                    data = list(dump_explorer_file(clicked_addr, os.path.join(current_path, 'Files With Type (Plugin Creation)')).values())
...
class WinObjExplorer(Explorer):
    ...
    NAME_TO_TYPE = {# 'ALPC Port:': '',
                    ...
                    'Mutant': '_KMUTANT',   # ---------------------------------
                    'Section': '_SECTION_OBJECT', # This object replace with _SUBSECTION in windows 10.
    ...
    def run_struct_analyze(self):
        ...
        if type not in WinObjExplorer.NAME_TO_TYPE:
            print('[-] WinObjExplorer dont support Struct Analyzer on this object ({})'.format(type))
            return
...
class StructExplorer(Explorer):
    """
    Explorer like gui for structs.
    # TODO -> improve function detection, class to do vol.offset (in the member value tab)
    """
    def __init__(self, master, dict, sa_self,  headers=("Struct Name", "Member Value", "Struct Address", "Object Type"), searchTitle='Search', writeSupport=False, relate=None, *args, **kwargs):
        ...
        self.relate = master
        self.layer_name = sa_self.config.PID if hasattr(sa_self.config, 'PID') and sa_self.config.PID else 4
        self.app = None

        # Add write support feature if enable.
        if writeSupport:
            self.tree.aMenu.add_command(label="Write", command=self.RunWrite)
        self.tree.aMenu.add_command(label='Cast To:', command=self._cast)
    ...
    def dereference(self):
        ...
        my_list = self.sa_self.context.symbol_space['nt_symbols1'].types#symbols.types#list(self.sa_self.kaddr_space.profile.vtypes.keys())
        ...
        if self.relate:
            ...
            app.geometry("+%d+%d" % (x + ABS_X, y + ABS_Y))
        app.geometry("500x50")
        app.title('Please choose your struct type')
    ...
    def _cast(self):
        ...
        # Open the SmartChoose to let the user pick an struct.
        app = SmartChoose(my_list, self.cast_func, default=my_list[my_list.index(default_struct)])
    ...
    def cast_func(self, app):
        ...
        struct_layer_name = self.layer_name
        ...
        if struct_type == 'Function':
            ...
            return
        elif struct_type.lower() in ('pointer', 'ex_fast_ref'):
            pass
        elif struct_type.upper() == 'EX_FAST_REF':
            pass

        # Dereference the struct.
        #struct = self.sa_self.context.object(struct_type, offset=struct_addr)#struct = obj.Object(struct_type, struct_addr, self.sa_self.kaddr_space)
        ...
        struct = struct.cast(app.dereference)
        ...
        if not struct or (hasattr(struct, 'is_valid') and not struct.is_valid()):
        ...
        run_struct_analyze(app.dereference, struct.vol.offset if (struct_type != app.dereference) and hasattr(struct, 'vol') else struct_addr, None, struct_layer_name)
        # Change the type in the explorer.
        #good_values = list(my_item['values'])
        #good_values[1] = struct.vol.offset if (good_values[-1] != app.dereference) and hasattr(struct, 'vol') else good_values[1] # obj.Object(app.dereference, good_values[1], self.sa_self.kaddr_space).v()
    ...
    def dereference_func(self, app):
        ...
        if self.sa_self.context.layers['memory_layer'].name == self.layer_name:
            ...
            struct = self.sa_self.context.object('nt_symbols1!' + struct_type, offset=int(struct_addr),
                                                 layer_name=self.layer_name)
        struct = struct.dereference().cast(app.dereference)
        ...
        # Pop up a new struct analyzer with the changes items:
        run_struct_analyze(app.dereference, struct.vol.offset if (struct_type != app.dereference) and hasattr(struct, 'vol') else struct_addr, None, struct_layer_name)
        # Change the type in the explorer.
        #good_values = list(my_item['values'])
        #good_values[1] = struct.vol.offset if (good_values[-1] != app.dereference) and hasattr(struct, 'vol') else good_values[1] # obj.Object(app.dereference, good_values[1], self.sa_self.kaddr_space).v()
    ...
    def OnDoubleClick(self, event):
        ...
        if struct_type == 'Function':
            ...
            if type(function) in (renderers.UnreadableValue, renderers.NotApplicableValue, renderers.UnparsableValue, renderers.NotAvailableValue):#isinstance(function, obj.NoneObject):
            ...
            def run_function_in_thread(self, function, arguments, s):
                ...
                if inspect.isgenerator(struct):
                    ...
                    for i in return_gen_list[0]:
                        ...
                        if isinstance(i, objects.StructType):
                            print('[+] find struct inside the result:', type(i))
                            find_object = True
                            _data = []
                            for j in return_gen_list:
                                _struct = j[return_gen_list[0].index(i)]
                                _data.append((_struct.vol.type_name.split(constants.BANG)[-1], _struct.vol.offset))
                            ...
                            def create_object_generator(_data, i, s):
                                ...
                                def run_new_sa(event):
                                    ...
                                    print(_my_item['values'][0], _my_item['values'][1], _self.layer_name)
                                    run_struct_analyze(_my_item['values'][0], _my_item['values'][1], pid=_self.layer_name)
                                ...
                                while p_struct.vol.parent != None:
                                    p_struct = p_struct.vol.parent
                                ...
                                if hasattr(p_struct, 'UniqueProcessId'):
                                    _treetable.tree.layer_name = p_struct.add_process_layer()#int(p_struct.UniqueProcessId)
                                else:
                                    _treetable.tree.layer_name = self.layer_name#4
                ...
                elif isinstance(struct, objects.StructType):
                    queue.put((run_struct_analyze, (struct.vol.type_name.split(constants.BANG)[-1], struct.vol.offset, None, self.layer_name)))
        ...
        # Get inside some member of this struct (double click on none function option)
            ...
            for key in list(pMftExp.keys()):
                if key != "|properties|":
                    ...
                    if len(pMftExp[key]) - ("|properties|" in pMftExp[key]) > 0:
                        directories.append(key)
                    my_tup = (key,) + my_tup if type(my_tup) == tuple else (key, str(my_tup))
                    data.append(my_tup)
...
class PopUp(tk.Toplevel):
    def __init__(self, options, func_to_call, *args, **kwargs):
        ...
        for item in options:
            self.dict[item] = tkinter.ttk.Entry(self)
            tkinter.ttk.Label(self, text=item, wraplength=500).pack(fill='x')
            self.dict[item].insert(10, options[item])
            self.dict[item].pack(fill='x')

        # Create and pack the button
        self.save_button = tkinter.ttk.Button(self, text="<- Save & Continue ->", command=self.Save)
...
class MessagePopUp(tk.Toplevel):
    ...
    def __init__(self, string_to_show, seconds_to_show=5, related=None, title='Informational', *args, **kwargs):
        ...
        tkinter.ttk.Label(self, text=string_to_show).pack(side=tk.LEFT)
...
class Options(Frame):
    ...
    def __init__(self, master, dict=None, on_start=True, *args, **kwargs):
        Frame.__init__(self, master, *args, **kwargs)
        self.data_order = (
        'Saved File', 'Memory File', 'Dump-Dir', 'Show Unnamed Handles',
        'Show PE Strings', 'Volatility File Path', 'Virus Total API Key')
        self.on_start = on_start
        self.master = master
        self.user_dict = dict
        self.buttons = {}

        self.options = options = {'Saved File': dict['Saved File'] if 'Saved File' in dict else '',
                                  'Memory File': dict['Memory File'] if 'Memory File' in dict else '',
                                  'Dump-Dir': dict['Dump-Dir'] if 'Dump-Dir' in dict else '',
                                  'Show Unnamed Handles': dict['Show Unnamed Handles'] if 'Show Unnamed Handles' in dict and dict['Show Unnamed Handles'].lower() in ['true',
                                                                                                           'false'] else 'False',
                                  'Show PE Strings': dict['Show PE Strings'] if 'Show PE Strings' in dict and dict['Show PE Strings'].lower() in ['true', 'false'] else 'True',
                                  'Volatility File Path': dict['Volatility File Path'] if 'Volatility File Path' in dict else (sys.argv[0] if ((
                                  ...
                                  'Virus Total API Key': dict['Virus Total API Key'] if 'Virus Total API Key' in dict else ''}
        self.dict = {}
        title = tkinter.ttk.Label(master, text='Options Menu:', wraplength=500, font=("Courier", 18))
        title.config(anchor="center")
        title.pack(fill='x')
        if on_start:
            tkinter.ttk.Label(master, text='you must fill all the (*) (or enter .atz file[saved from some run])', wraplength=500,
                     font=("Courier", 14)).pack(fill='x')
        else:
            tkinter.ttk.Label(master,
                     text='Load a saved .atz file or enter the setting manually:',
                     wraplength=500, font=("Courier", 14)).pack(fill='x')

        # Get Saved File: (*)
        self.dict['Saved File'] = tkinter.ttk.Entry(master)#, state='normal' if on_start else 'disabled')
        self.buttons['Saved File'] = tkinter.ttk.Button(master,
        ...
        # Get Memory File: (*)
        self.dict['Memory File'] = tkinter.ttk.Entry(master)
        self.buttons['Memory File'] = tkinter.ttk.Button(master, text='Memory File (*)',
                                                state='normal' if on_start else 'disabled',
        ...
        # Get Dump directory (*)
        self.dict['Dump-Dir'] = tkinter.ttk.Entry(master)
        self.buttons['Dump-Dir'] = tkinter.ttk.Button(master, text='Dump-Dir (*)', command=lambda: self.get_dir('Dump-Dir'))
        ...
        self.dict['Volatility File Path'] = tkinter.ttk.Entry(master)
        self.buttons['Volatility File Path'] = tkinter.ttk.Button(master, text='Volatility File Path (*)',
        ...
        self.buttons['Show Unnamed Handles'] = tkinter.ttk.Label(master, text='Show Unnamed Handles', wraplength=500)
        ...
        self.buttons['Show PE Strings'] = tkinter.ttk.Label(master, text='Show PE Strings', wraplength=500)
        ...
        self.dict['Virus Total API Key'] = tkinter.ttk.Entry(master)
        self.buttons['Virus Total API Key'] = tkinter.ttk.Label(master,
...
class LoadingButton(tk.Button):
    ...
    def display_running(self, event=None):
        ...
        top_frame = tkinter.ttk.Frame(self.pw)
        bot_frame = tkinter.ttk.Frame(self.pw)

        # Create the top Frame
        label = tkinter.ttk.Label(top_frame, text="Here you can get information about tasks that are currently running.")
        label.pack(side="top", fill="x", pady=10)
        ...
        label = tkinter.ttk.Label(bot_frame, text="Here you can get information about tasks that this finished running.")
        label.pack(side="top", fill="x", pady=10)
        headers = ("Job ID", "Job Name", "Job Args", "Job Status", "Job Start Time")
        ...
        self.refresh_button = tkinter.ttk.Button(self.app, text='Refresh', command=self.refresh_data)
...
class Search(tk.Toplevel):
    ...
    def OnDoubleClick(self, event):
        ...
        for row in self.main_table.get_all_children(self.main_table.tree):
            ...
            if 'values' in self.main_table.tree.item(row):
...
class CmdPlugin(tk.Toplevel):
    ...
    '''
    def __init__(self, plugin_name, vol_path, plugins_path, file_path,  *args, **kwargs):
        tk.Toplevel.__init__(self, *args, **kwargs)
        self.headers = ('Result',)

        # Set the self.default_plugin
        self.default_plugin = r'"{}" "{}" -p "{}" -f "{}" {}'.format(sys.executable, vol_path, plugins_path, file_path, plugin_name)
        ...
        self.search_text.pack(fill='x')
        self.running = False
        frame_line = tkinter.ttk.Frame(self)
        self.run_button = tkinter.ttk.Button(frame_line, text="Run-->>>", command=lambda: self.run(None))
        ToolTip(self.run_button, 'Run This Plugin')
        self.clear_button = tkinter.ttk.Button(frame_line, text="Clear Screen", command=lambda: self.clear(None))
        ToolTip(self.clear_button, 'Clean Table From All the Rows')
        self.apply_button = tkinter.ttk.Button(frame_line, text="Apply (on properties)", command=lambda: self.apply(None))
        ToolTip(self.apply_button, 'Adds The Output of the Last Used Plugin Run\nAs a New Tab for Each Affected Process')
        self.cb = tkinter.ttk.Checkbutton(frame_line, text='Alert processes (Ctrl+U to unalert)', state='selected')
    ...
    def apply(self, event):
        ...
        apply_header = tkinter.simpledialog.askstring(title="Apply Header", prompt="Please enter the tab name for this appliance:", parent=self)
        ...
        for head in self.headers:
            if head.strip('"').strip("'").strip('-').strip('.').strip('_').strip(':').strip(' ').lower() in ('pid', 'processid'):
                header_id = self.headers.index(head)

        if header_id == -1:
            ans = tkinter.simpledialog.askstring(title="Notice",
                                            prompt="unnable to auto detect pid header please enter the header index to search for (from 0 to {})".format(len(self.headers)-1), parent=self)
        ...
        for c_row in self.data:
            ...
            for pid in process_performance:
                if str(pid) == row.strip('"').strip("'").lower():
        ...
        for c_pid in data_dict:
            if data_dict[c_pid] != '':
                ...
                if c_pid not in plugins_output:
    ...
    def run(self, event):
        ...
        if not '-r csv' in self.default_plugin:
            self.default_plugin = self.default_plugin.replace('vol.py"', 'vol.py" -r csv')
            print('default', self.default_plugin)
        self.clear(None)
    ...
    def insert_item_thread(self):
        ...
        output = output[output.find('"TreeDepth",') + len('"TreeDepth",'):]
        print(output)
        ...
        def try_insert_data():
            ...
            try:
                ...
                self.headers =[head.strip('"') for head in lines[0].split(',')]
                self.data = []
                c_line = []
                c_data = ''
                even = False

                # Go all over the data from the result and parse it as good as csv
                for char in '\r\n'.join(lines[1:]):

                    # Add another entry to the table
                    if len(c_line)-1 == len(self.headers) and not even:
                        c_line[-1] = c_line[-1][:c_line[-1].rfind('\r\n')]
                        self.data.append(c_line[1:])
                        c_line = []
                        c_data = c_data.split('\r\n')[-1] + ','
                        c_line.append(c_data)
                        c_data = ''
                        even = False

                    # Make sure we parse it right
                    if char == ',' and not even:
                        c_line.append(c_data)
                        c_data = ''
                    elif char == '"':
                        even = not even
                ...
                #	print(type(dat), type(self.headers))
                #	print(dat)
                #	if len(dat[0]) != len(self.headers):
                ...
                def create_thread_result_ui():
                    ...
                    try:
                        ...
                        messagebox.showinfo('Plugin Finish Running',
                                            'You exit/clear the CmdPlugin window but you can still view the output in your shell.')
                    except tk.TclError:
                        messagebox.showinfo('Plugin Finish Running',
...
class About(tk.Toplevel):
    ...
    def __init__(self, img, *args, **kwargs):
        ...
        image_frame = tkinter.ttk.Frame(self)
        about_frame = tkinter.ttk.Frame(self)

        # Insert all the data to the toplevel and pack it.
        tkinter.ttk.Label(image_frame, image=img).pack()
        tkinter.ttk.Label(about_frame, text="Volatility Explorer\nCreator: Aviel Zohar\nContact: memoryforensicsanalysis@gmail.com", compound=tk.CENTER).pack(side=TOP)
        git_link = tkinter.ttk.Label(about_frame, text="Go To Github", foreground='blue', compound=tk.CENTER)
        ToolTip(git_link, r"https://github.com/memoryforensics1/Vol3xp")
        git_link.bind("<Button-1>", lambda e: open_new(r"https://github.com/memoryforensics1/Vol3xp"))
        git_link.pack(side=TOP)
        tkinter.ttk.Label(about_frame, text=CREDITS, compound=tk.CENTER).pack(side=BOTTOM)
...
class TreeTable(Frame):
    ...
    def header_selected(self,event=None):
        ...
        if self.app_header:
            ...
            x = self.master.winfo_x()#x = self.winfo_x()
            y = self.master.winfo_y()#y = self.winfo_y()
...
class TreeLable(TreeTable):
    ...
    def __init__(self, master, headers, data, name=None, text_by_item=0, resize=False, display=None, *args, **kwargs):
        ...
        # Class variables
        self.opts = {}
        self.frame = frame = tkinter.ttk.Frame(master)
        self.frames = []
        ...
        for item in headers:
            if counter % col_size == 0:
                papa = tkinter.ttk.Frame(frame)
                self.frames.append((tkinter.ttk.Frame(papa), tkinter.ttk.Frame(papa), papa))

            # Create a label to each header.
            tkinter.ttk.Label(self.frames[counter//col_size][0], text='{} : '.format(item), wraplength=500).pack(anchor='w', padx=10)
            self.opts[item] = StringVar()
            self.opts[item].set('-')
            tkinter.ttk.Label(self.frames[counter//col_size][1], textvariable=self.opts[item]).pack(anchor='w')
...
class DllsTable(TreeTable):
    ...
    def __init__(self, master, main_table, headers, data, name=None, text_by_item=0, resize=False, display=None, pid=None):
        ...
        #self.hexdump_menu.add_command(label='MemHexDump', command=self.MemHex) #FixMe
        ...
        self.colors_menu.add_separator()
        self.colors_menu.add_command(label='Custom Color', command=lambda: self.SetColor(_from_rgb(tkinter.colorchooser.askcolor()[0])))
        self.aMenu.add_cascade(label='HexDump', menu=self.hexdump_menu)
        self.aMenu.add_command(label='Struct Analyzer', command=lambda: self.run_struct_analyze('_LDR_DATA_TABLE_ENTRY'))
        self.aMenu.add_separator()
        ...
        with lock:
            self.context = volself.context.clone()
            self._primary = volself.config['primary']
            self._config_path = volself.config_path
            self._nt_symbols = volself.config['nt_symbols']
            self.kvo = self.context.layers[self._primary].config["kernel_virtual_offset"]
    ...
    def DllDump(self):
        ...
        task_pid = self.pid or int(self.main_table.tree.item(item)['values'][self.main_table.text_by_item])
        dllitem = self.tree.selection()[0]

        # Check if we have the address of this pe.
        if self.tree.item(dllitem)['values'][self.text_by_item] not in process_bases[self.pid or int(self.main_table.tree.item(item)['values'][self.main_table.text_by_item])]["dlls"]:
            def show_message_func():
        ...
        try:
            df_conf = self.context.clone()
            pe_table_name = intermed.IntermediateSymbolTable.create(df_conf,
                                                                    self._config_path,
                                                                    "windows",
                                                                    "pe",
            ...
            # Find the right task.
            for task in tasks.PsList.list_processes(df_conf, self._primary, self._nt_symbols):
                if task.UniqueProcessId == task_pid:
                    break
            proc_layer_name = task.add_process_layer()
            gen = task.load_order_modules() if self.pid != 4 else modules.Modules.list_modules(df_conf, self._primary, self._nt_symbols)
            for dllEntry in gen:
                if dllEntry.DllBase.real == self.tree.item(dllitem)['values'][-2]:#process_bases[task_pid]["dlls"][str(dllEntry.BaseDllName.String)]:
                    break
            dos_header = df_conf.object(pe_table_name + constants.BANG + "_IMAGE_DOS_HEADER",
                                        offset=dllEntry.DllBase,
                                        layer_name=proc_layer_name)
            try:
                name = dllEntry.BaseDllName.String
            except exceptions.InvalidAddressException:
    ...
    def PEHex_thread(self, mt_item, c_item, mem=False):
        ...
        # Check if we have the address of this pe.
        if c_item['values'][self.text_by_item] not in process_bases[self.pid or int(mt_item['values'][self.main_table.text_by_item])]["dlls"]:
            def show_message_func():
                messagebox.showerror("Error", "Unable to locate this PE address in memory)", parent=self)
        ...
        try:
            df_conf = self.context.clone()
            task_pid = self.pid or int(mt_item['values'][self.main_table.text_by_item])
            pe_table_name = intermed.IntermediateSymbolTable.create(df_conf,
                                                                    self._config_path,
                                                                    "windows",
                                                                    "pe",
            ...
            for task in tasks.PsList.list_processes(df_conf, self._primary, self._nt_symbols):
                if task.UniqueProcessId == task_pid:
                    break
            proc_layer_name = task.add_process_layer()
            gen = task.load_order_modules() if self.pid != 4 else modules.Modules.list_modules(df_conf, self._primary,
                                                                                               self._nt_symbols)
            for dllEntry in gen:
                if dllEntry.DllBase.real == c_item['values'][-2]:  # process_bases[task_pid]["dlls"][str(dllEntry.BaseDllName.String)]:
                    break
            dos_header = df_conf.object(pe_table_name + constants.BANG + "_IMAGE_DOS_HEADER",
                                        offset=dllEntry.DllBase,
                                        layer_name=proc_layer_name)
    ...
    def Properties(self, event, top_level=True):
        ...
        if not has_pefile:
            messagebox.showerror("Error", "This function need pefile module to parse the file, please install pe file and run this plugin again", parent=self)
            return
    ...
    def properties_thread(self, event=None, mt_item=None, c_item=None, top_level=True):
        ...
        pe_conf = volself.context.clone()
        pe_conf.kaddr = volself.config['primary']
        kvo = pe_conf.layers[pe_conf.kaddr].config["kernel_virtual_offset"]
        ntkrnlmp = pe_conf.module(volself.config['nt_symbols'], layer_name=pe_conf.kaddr,
                                            offset=kvo)
        pe_table_name = intermed.IntermediateSymbolTable.create(pe_conf,
                                                                volself.config_path,
                                                                "windows",
                                                                "pe",
                                                                class_types=extensions.pe.class_types)
        ...
        task = ntkrnlmp.object("_EPROCESS", offset=process_bases[self.pid or int(mt_item['values'][self.main_table.text_by_item])]["proc"].vol.offset - kvo)
        task_space = pe_conf.kaddr if (not self.pid) or self.pid == 4 else task.add_process_layer()
        ...
        pe_file = pe_conf.object(pe_table_name + constants.BANG + "_IMAGE_DOS_HEADER", offset=module, layer_name=task_space)

        # Get nt_header
        try:
            nt_headers = pe_file.get_nt_header()
        except exceptions.InvalidAddressException:
        ...
        if hasattr(pe, 'DIRECTORY_ENTRY_IMPORT'):
            imports = [(imp.name.decode(errors='replace') if imp.name else 'None',
                                          entry.dll.decode(errors='replace') if entry.dll else 'None',
                                          hex(imp.address) if imp.address else hex(-1),
                                          hex(imp.hint) if imp.hint else hex(-1)) for entry in pe.DIRECTORY_ENTRY_IMPORT for
                                         imp in entry.imports]

        # Get Exports
        pe.parse_data_directories(directories=[pefile.DIRECTORY_ENTRY['IMAGE_DIRECTORY_ENTRY_EXPORT']])
        if hasattr(pe, 'DIRECTORY_ENTRY_EXPORT'):
            exports = [(hex(exp.ordinal) if exp.ordinal else hex(-1),
                                          hex(pe.OPTIONAL_HEADER.ImageBase + exp.address) if exp.address else hex(-1),
                                          exp.name.decode(errors='replace') if exp.name else 'None')
                                          for exp in pe.DIRECTORY_ENTRY_EXPORT.symbols] #index, address, name



        # Get the strings if the user wants to
        if volself._show_pe_strings == 'true': #FIXME
        ...
        # Get dlls / modules #FIXME
        lm = list(modules.Modules.list_modules(pe_conf, pe_conf.kaddr, volself.config['nt_symbols'])) if self.pid==4 or int(mt_item['values'][self.main_table.text_by_item])==4 else list(task.load_order_modules())
        #imports = []
        #exports = []
        ...
        my_data = [pe_file, imports, exports, mem_strings, image_strings, c_module, self.pid or int(mt_item['values'][self.main_table.text_by_item]), nt_headers]
    ...
    def set_saved_color(self):
        ...
        for child in self.get_all_children(self.tree):
            ...
            if len(self.headers) == 12:
                fn, description, cn, _, _, _, _, _, _, path, dll_base, ldr_addr = self.tree.item(child)['values']
            else:
            ...
            # unfix the treetable
            if path not in pe_comments['pid'][pid]:
...
class ProcessesTable(TreeTable):
    ...
    def __init__(self, master, headers, data, name=None, text_by_item=0, resize=False, display=None, disable_header_replace=600, folder_by_item=0, folder_text="?/?"):
        ...
        well_known_plugins = ["windows.malfind.Malfind"]
        ...
        self.aMenu.add_command(label='Struct Analyzer', command=lambda: self.run_struct_analyze('_EPROCESS'))
        self.aMenu.add_cascade(label='Properties', menu=self.properties_menu)
        ...
        with lock:
            ...
            self._nt_symbols = volself.config['nt_symbols']
            self.kvo = self.context.layers[self._primary].config["kernel_virtual_offset"]
        self.ntkrnlmp = self.context.module(self._nt_symbols, layer_name=self._primary,
                                                    offset=self.kvo)
        self.task_list = list(tasks.PsList.list_processes(self.context, self._primary, self._nt_symbols))
    ...
    def run_struct_analyze(self, struct_type):
        ''' get address and sent to teal Struct Analyzer function. '''
        item = main_table.tree.selection()[0]
        pid = self.tree.item(item)['values'][1]
        addr = self.tree.item(item)['values'][-1]
        if not addr:
            print("[-] unable to find the address of this {}".format(struct_type))
            return
        print("[+] Run Struct Analyzer on {}, addr: {}".format(struct_type, addr))
        threading.Thread(target=run_struct_analyze, args=(struct_type, addr, None, pid)).start()
    ...
    def set_saved_color(self):
        ...
        for child in self.get_all_children(self.tree):
            ...
            if pid in process_comments['pidColor']:
                item = child
    ...
    def run_plugin(self, plugin_name):
        ...
        app.resizable(False, False)
        label = tkinter.ttk.Label(app, text="Title:")
        label.pack()
        txt_entry = tkinter.ttk.Entry(app, width=380)
        ...
        command = r'"{}" "{}" -f "{}" {} --pid {}'.format(sys.executable, volself._vol_path, file_path, plugin_name, pid)
        ...
        apply = tkinter.ttk.Button(app, text="Run & Update Properties", command=run_plugin_thread)
        apply.pack()
    ...
    def ProcDump(self, event=None):
        ...
        threading.Thread(target=self.ProcDumpThread, args=(int(self.tree.item(self.tree.selection()[0])['values'][self.text_by_item]), )).start()
        time.sleep(1)

    def ProcDumpThread(self, pid):
        '''
    ...
    def MemHex(self):
        ...
        threading.Thread(target=self.ProcHexThread, args=(int(self.tree.item(self.tree.selection()[0])['values'][self.text_by_item]), True)).start()
        time.sleep(1)
    def ImageHex(self):
        ''' This function summon the ProcHexThread in a thread with the mem=False (default)'''
        threading.Thread(target=self.ProcHexThread, args=(int(self.tree.item(self.tree.selection()[0])['values'][self.text_by_item]), )).start()
        time.sleep(1)
    def ProcHexThread(self, pid, mem=False): #FIXME mem true/false
        '''
    ...
    def process_memory(self, event=None):
        ...
        MessagePopUp('Data loading in the background\nThis will going to take a while because it\'s go all over the VAD\'s', 3.5, root)
        time.sleep(1)
    def process_memory_thread(self, task):
        ...
        id = time.time()
        job_queue.put_alert((id, 'Memory Information', 'get all the memory information - {} ({})'.format(objects.utility.array_to_string(task.ImageFileName), int(task.UniqueProcessId)), 'Running'))
        self.lock.acquire()

        # Get process from another conf (so we can run in thread).
        for c_task in self.task_list:
            if c_task.vol.offset == task.vol.offset:
                task = c_task
                break
        ...
        other_memory_range = {}
        try:
            task_peb = task.get_peb()
        except exceptions.InvalidAddressException:
            task_peb = None
        if task_peb:
            try:
                heaps_count = task_peb.NumberOfHeaps
            except exceptions.InvalidAddressException:
                heaps_count = 0
            #heapArr = self.ntkrnlmp.object(object_type='array', offset=task_peb.ProcessHeaps.dereference() - self.kvo, subtype=self.ntkrnlmp.get_type('_HEAP'), count=heaps_count, layer_name=addr_space)
            #heapArr = self.ntkrnlmp.object(object_type='array', offset=task_peb.ProcessHeaps.dereference() - self.kvo, subtype=self.ntkrnlmp.get_type('_HEAP'), count=heaps_count, layer_name=addr_space, native_layer_name=addr_space)

            # Go all over the heaps to get the addresses of the head and the segments.
            for heapIndex in range(heaps_count):#heapArr:
                try:
                    heapAddr = task_peb.ProcessHeaps.dereference() + (self.ntkrnlmp.get_type('_HEAP').size * heapIndex)
                    heap = self.context.object(task.vol.type_name.split(constants.BANG)[0] + constants.BANG + "_HEAP", offset=heapAddr, layer_name = addr_space, native_layer_name = addr_space)
                    heaps[heapAddr] = heap.ProcessHeapsListIndex
                    heaps[heap.vol.offset] = heap.ProcessHeapsListIndex
                except exceptions.InvalidAddressException:
                    continue
                # Don't check on xp. (todo-> Check the version instead)
                if hasattr(heap, 'SegmentListEntry'):
                    for heap_segment in list(heap.SegmentListEntry.to_list("{}{}_HEAP_SEGMENT".format(task.get_symbol_table_name(), constants.BANG), "SegmentListEntry")):
                        heaps_segment[int(heap_segment.vol.offset)] = heap.ProcessHeapsListIndex #Fixme to add virtual and physical like above in the heaps[]
                        #offset = self.context.layers[heap_segment.vol.layer_name].translate(heap_segment.vol.offset, True)
                        #if offset[0]:
                        #heaps_segment[heap_segment] = heap.ProcessHeapsListIndex  # Fixme to add virtual and physical like above in the heaps[]



            # This is all other memory types.
            other_memory_range = {task_peb.vol.offset: 'PEB',
                                  task_peb.ReadOnlySharedMemoryBase: 'Read Only Memory Base', # Shared / static server data...
                                  task_peb.GdiSharedHandleTable: 'Gdi Shared Handle Table',
                                  task_peb.AnsiCodePageData: 'Ansi Code Page Data',
                                  task_peb.pShimData: 'Shim Data',
                                  0x7ffe0000: 'User Shared Data'
                                  }

            # todo-> Check the version instead:
            if hasattr(task_peb, 'LeapSecondData'):
                other_memory_range.update({task_peb.LeapSecondData.vol.offset: 'Leap Second Data'})

        # Go all over the threads and get stack information.
        for thread in task.ThreadListHead.to_list("{}{}_ETHREAD".format(task.get_symbol_table_name(), constants.BANG), "ThreadListEntry"):
            teb = self.context.object(task.vol.type_name.split(constants.BANG)[0] + constants.BANG + "_TEB", offset=thread.Tcb.Teb, layer_name = addr_space, native_layer_name = addr_space)#thread.Tcb.Teb.dereference().cast('_TEB')#self.context.object(task.vol.type_name.split(constants.BANG)[0] + constants.BANG + "_TEB", offset=thread.Tcb.Teb, layer_name = task.vol.layer_name, native_layer_name = task.vol.native_layer_name)#obj.Object("_TEB", offset=thread.Tcb.Teb, vm=task.get_process_address_space())
            try:
                # Check terminated thread (also with exit time).
                if 'PS_CROSS_THREAD_FLAGS_TERMINATED' in str(thread.get_cross_thread_flags()) or thread.ExitTime.QuadPart > 0: #FIXME!
                    status = 'Terminate'
                else:
                    # If thread is waiting get also the wait reason
                    if thread.Tcb.State == 5:
                        status = 'Wait ({})'.format(str(thread.Tcb.get_wait_reason()))
                    else:
                        status = str(thread.Tcb.get_state())

                # Stack Commit memory
                stacks[teb.NtTib.StackBase] = 'Stack Base (tid: {}) [{}]'.format(int(thread.Cid.UniqueThread), status)

                # From StackLimit to here is the resered memory to the stack.
                stacks[teb.DeallocationStack] = 'Stack Reserved | Stack Guard start from {} (tid: {}) [{}]'.format(hex(teb.NtTib.StackLimit.vol.offset), int(thread.Cid.UniqueThread), status)
            except:
                pass # invalid teb
        ...
        for vad in task.get_vad_root().traverse():
            if vad != None:
                ...
                protection = vad.get_protection(
                    vadinfo.VadInfo.protect_values(self.context, self._primary, self._nt_symbols),
                    vadinfo.winnt_protections)


                # XP dont have this field.
                c_vad_flags = get_right_member(vad, ["u.VadFlags", "Core.u.VadFlags"])
                if hasattr(c_vad_flags, "VadType"):
                    vadType = c_vad_flags.VadType
                    vadFlags = str(list(c_vad_flags.__dict__.values())[2])[1:-1]



                try:
                    if vad.has_valid_member('ControlArea'):
                        control_area = vad.ControlArea.dereference()
                    else:
                        control_area = vad.Subsection.dereference().ControlArea.dereference()

                    # Check that this is not private memory
                    if c_vad_flags.PrivateMemory != 1 and control_area:
                        if control_area:
                            controlAreaAddr = control_area.vol.offset
                            segmentAddr = control_area.Segment.vol.offset
                            ...
                            for flag in dir(control_area.u.Flags):
                                if flag[0].isupper() and flag!='VolTemplateProxy' and getattr(control_area.u.Flags, flag):
                                    controlFlags += '{}, '.format(flag)
                            controlFlags = controlFlags[:-2] if len(controlFlags) else ''
                            file_object = control_area.FilePointer

                            if file_object:
                                fileObjectAddr = file_object.vol.offset
                                use = vad.get_file_name()

                                if type(use) in (renderers.UnreadableValue, renderers.NotApplicableValue, renderers.UnparsableValue, renderers.NotAvailableValue):
                                    use = ''
                ...
                if use == '':
                    if vad.get_start() in other_memory_range:
                        use = other_memory_range[vad.get_start()]
                    elif vad.get_start() in stacks:
                        use = stacks[vad.get_start()]
                    elif vad.get_start() in heaps:
                        use = 'Heap (ID {})'.format(heaps[vad.get_start()])
                    elif vad.get_start() in heaps_segment:
                        use = 'Heap Segment (ID {})'.format(heaps_segment[vad.get_start()])

                try:
                    firstPrototypePteAddr = (get_right_member(vad, ["vad.FirstPrototypePte", "vad.Core.FirstPrototypePte"]) or 0)
                    lastContiguousPteAddr = (get_right_member(vad, ["vad.LastContiguousPte", "vad.Core.LastContiguousPte"]) or 0)
                    for flag in dir(vad.u2.VadFlags2):
                        if flag[0].isupper() and flag!='VolTemplateProxy' and getattr(vad.u2.VadFlags2, flag):
                            flags2 += '{}, '.format(flag)
                    flags2 = flags2[:-2] if len(flags2) > 0 else ''
                except AttributeError:
                    pass
                vad_data.append([hex(vad.get_start()),
                                 hex(vad.get_end()),
                                 hex(vad.get_end() - vad.get_start()).replace('L', ''),
                                 vad.get_tag(),#{} ({})".format(vad.Tag, vad.tag_map[str(vad.Tag)]) if vad.Tag else ''),
                                 str(vadFlags),#.VadFlags or ''),
                                 str(protection),
                                 str(vadType),
                                 hex(controlAreaAddr),
                                 hex(segmentAddr),
        ...
        job_queue.put_alert((id, 'Memory Information', 'get all the memory information - {} ({})'.format(objects.utility.array_to_string(task.ImageFileName), int(task.UniqueProcessId)), 'Done'))
    def display_mem_info(self, data, task):
        ...
        "Last Contiguous Pte", "Flags2", "Vad Address"), data=data, display=('Start', 'Size', "Use", "Protection", "Control Flags"))
        treetree.pack(expand=YES, fill=BOTH)
        treetree.self = self
        app.geometry("750x500")
        app.title("{} ({}): Memory information (from VAD)".format(objects.utility.array_to_string(task.ImageFileName), int(task.UniqueProcessId)))
        self.lock.release()
    ...
    def show_lower_pane(self, frame_to_show=None):
        ...
        for item in data:
            ...
            if "Failed to get dll name on address: " in item:
                ...
                cn = Description = FileVersion = InternalName = LegalCopyright = OriginalFilename = ProductName = ProductVersion = ""
                if str(item).lower() in files_info:
                    if "CompanyName" in files_info[str(item).lower()]:
                        cn = files_info[str(item).lower()]["CompanyName"]
                    if "FileDescription" in files_info[str(item).lower()]:
                        Description = files_info[str(item).lower()]["FileDescription"]
                    if "FileVersion" in files_info[str(item).lower()]:
                        FileVersion = files_info[str(item).lower()]["FileVersion"]
                    if "InternalName" in files_info[str(item).lower()]:
                        InternalName = files_info[str(item).lower()]["InternalName"]
                    if "LegalCopyright" in files_info[str(item).lower()]:
                        LegalCopyright = files_info[str(item).lower()]["LegalCopyright"]
                    if "OriginalFilename" in files_info[str(item).lower()]:
                        OriginalFilename = files_info[str(item).lower()]["OriginalFilename"]
                    if "ProductName" in files_info[str(item).lower()]:
                        ProductName = files_info[str(item).lower()]["ProductName"]
                    if "ProductVersion" in files_info[str(item).lower()]:
                        ProductVersion = files_info[str(item).lower()]["ProductVersion"]
                good_data.append((item[item.rfind('\\')+1:], cn, Description, FileVersion, InternalName, LegalCopyright, OriginalFilename, ProductName, ProductVersion, item) + ((process_bases[pid]['dlls'][item[item.rfind('\\')+1:]], process_bases[pid]['ldr'][item[item.rfind('\\')+1:]]) if item[item.rfind('\\')+1:] in process_bases[pid]['dlls'] else (-1, -1)))

        # Create the dll data table.
        self.dlls_table = dlls_table = DllsTable(self.lower_table, self, headers=["Name", "Company Name", "Description", "File Version", "Internal Name", "Legal Copyright", "Original File Name", "Product Name", "Product Version", "Path", "Dll Base", "Ldr Address"], data=good_data, resize=False, display=("Name", "Company Name", "Description", "Path"))
        ...
        # Handles
        item = self.tree.selection()[0]
        if int(self.tree.item(item)['values'][self.text_by_item]) in process_handles:
            ...
            if 'process_handles' in done_run and done_run['process_handles']:
        ...
        data = [tup for tup in process_connections[int(self.tree.item(item)['values'][self.text_by_item])]] if int(self.tree.item(item)['values'][self.text_by_item]) in process_connections else [
...
class Vol3xp(interfaces.plugins.PluginInterface):
    ...
    def __init__(self, *args, **kwargs): # Fix init from vol2
        ...
        # Support old version
        try:
            self._DBGKD_GET_VERSION64 = info.Info.get_version_structure(self.context, self.config['primary'], self.config['nt_symbols'])
        except Exception:
            class Dummy(object):pass
            self._DBGKD_GET_VERSION64 = Dummy()
            self._DBGKD_GET_VERSION64.MinorVersion = self._DBGKD_GET_VERSION64.MajorVersion = 0

        # Set optional args
        self._config.SAVED_FILE = self.config.get('SAVED_FILE', None)
        self._config.DUMP_DIR = self.config.get('DUMP_DIR', None)
        self._config.API_KEY = self.config.get('API_KEY', '')

        self._config.PROFILE = "Major: {} Minor: {}, Build: {}".format(self._KUSER_SHARED_DATA.NtMajorVersion, self._KUSER_SHARED_DATA.NtMinorVersion, self._DBGKD_GET_VERSION64.MinorVersion)
        self.os_version = {"Major": int(self._KUSER_SHARED_DATA.NtMajorVersion),
                           "Minor": int(self._KUSER_SHARED_DATA.NtMinorVersion),
                           "Build": int(self._DBGKD_GET_VERSION64.MinorVersion)}
        print('PROFILE:', self._config.PROFILE)
        self._config.LOCATION = urllib.request.url2pathname(self.context.config['automagic.LayerStacker.single_location'][file_slice:])
        self.is_memtriage = False #TOFIX remove all of them.
        self._show_unnamed_handles = 'false'
        self._show_pe_strings = 'true'

        # If we running using a saved file.
        if self._config.SAVED_FILE:
            self.calc_return = list(self.load(self._config.SAVED_FILE))
        ...
        if not hasattr(self, '_vol_path'):
            self._vol_path = os.path.join(os.getcwd().lower().replace('volatility\\framework\\plugins\\windows', r''),'vol.py')  # sys._getframe(1).f_globals.get('__name__')#os.path.abspath(__file__)
        # Fix Subsection object name for windows 10
        if self.os_version['Major'] == 10:
            WinObjExplorer.NAME_TO_TYPE['Section'] = '_SUBSECTION'

        if not (self._config.SAVED_FILE or self._config.DUMP_DIR or self._config.API_KEY):
            root = Tk()
            self.img = tk.PhotoImage(data=ICON)
            root.iconphoto(True, self.img)
            self.popup_options(root)
            root.subprocess = loading_start('Initial Plugin')
        elif root == None:
            root = Tk()
            self.img = tk.PhotoImage(data=ICON)
        ...
        # Find the security json path (or raise error if its not in the plugin directory).
    ...
    def get_requirements(cls):
        return [
            ...
            requirements.StringRequirement(name = 'DUMP_DIR',
                                           ...
                                           default = None,
                                           optional = True),
            requirements.StringRequirement(name='SAVED_FILE',
    ...
    def get_all_handles(self):
        ...
        def check_special_types(type, value):
            ...
            if type == 'File':
                ...
                # To add this specific types of file object in memory, (there is more)
                #elif r'\Device\Tcp' == value:
                #	return FILE_ACCESS['TCP_ACCESS']
                #elif r'Device\Udp'
                #	return FILE_ACCESS['TCP_ACCESS']
        ...
        handles_conf = self.context.clone()
        handles_plug = handlesplugin.Handles(handles_conf, self.config_path)
    ...
    def get_all_plugins(self):
        ...
        # Get all plugins
        plugins = framework.list_plugins()#registry.get_plugin_classes(commands.Command, lower=True)
        ...
        all_plugins[0] =  self._config.get('plugins', None) or constants.PLUGINS_PATH #or self._vol_path.replace(file_name, os.path.join('volatility', 'plugins'))# plugins_dir if plugins_dir != '' else os.path.dirname(os.path.abspath(__file__))
        all_plugins[1].sort()
    ...
    def return_pstree(self):
        ...
        proc_list = []
        pslist = {}
        pss = tasks.PsList.list_processes(self.context, self.config['primary'], self.config['nt_symbols'])
        ...
        def draw_branch(pad, inherited_from, inherited_time):
            ...
            for task in list(pslist.values()):
                ...
                task_time = task.get_create_time()
                task_time = float("-inf") if type(task_time) in self.VolatilityNoneType else task_time.utcnow().timestamp()
                if task.InheritedFromUniqueProcessId == inherited_from and inherited_time <= task_time:

                    proc_list.append((task, pad))

                    # Added Because Python Ituration Problem(the try except removed).
                    if int(task.UniqueProcessId) in pslist:
                    ...
                    draw_branch(pad + 1, task.UniqueProcessId, task_time)

        # Go all over the processes and create the process tree in the dictionary.
        while len(list(pslist.keys())) > 0:
            keys = list(pslist.keys())
            root = find_root(pslist, keys[0])
            ctime = int(pslist[root].get_create_time().utcnow().timestamp()) if root in pslist else float("-inf")#int(time.time())
    ...
    def get_proc_verinfo(self):
        ...
        if not has_pefile:
            vollog.log(constants.LOGLEVEL_VVV, "files info need pefile module to parse the file, please install pe file and run this plugin again")
            return
        ...
        verinfo_conf = self.context.clone()
        verinfo_conf.kaddr = volself.config['primary']
        kvo = verinfo_conf.layers[verinfo_conf.kaddr].config["kernel_virtual_offset"]
        ntkrnlmp = verinfo_conf.module(self.config['nt_symbols'], layer_name=verinfo_conf.kaddr,
                                  offset=kvo)
        task_list = tasks.PsList.list_processes(verinfo_conf, self.config['primary'], self.config['nt_symbols'])
        c_app = None
        pe_table_name = intermed.IntermediateSymbolTable.create(verinfo_conf,
                                                                self._config_path,
                                                                "windows",
                                                                "pe",
                                                                class_types=extensions.pe.class_types)
        ...
        for task in task_list:
            process_addr_space = task.add_process_layer()

            my_modules = task.load_order_modules() if task.UniqueProcessId != 4 else \
                modules.Modules.list_modules(verinfo_conf, self.config['primary'], self.config['nt_symbols'])
            # Go all over the process modules
            for my_module in my_modules:

                try:
                    mod_path = my_module.FullDllName.String
                    dos_header = verinfo_conf.object(pe_table_name + constants.BANG + "_IMAGE_DOS_HEADER",
                        ...
                        if fileinfo.Key == b'StringFileInfo':
                            for st in fileinfo.StringTable:
                                for entry in st.entries.items():
                                    files_info[str(my_module.FullDllName.String).lower()][entry[0].decode()] = entry[1].decode()
                        #if fileinfo.Key == b'VarFileInfo': # TOADD ?
                        #   for var in fileinfo.Var:
                        #        print(var.entry.items()[0])
    ...
    def update_connections(self):
        ...
        my_connections_conf = self.context.clone()
        ...
        if self.os_version['Major'] < 6:
            ...
            my_connections = my_connections_plug._generator()

            # Go all the network information from netscan
            for none, (offset, protocol, local_address, local_port, remote_address, remote_port, state, pid, proc_name, create_time) in my_connections:
                pid = '-' if type(pid) in self.VolatilityNoneType else str(pid)
                #proc_name = '-' if type(proc_name) in (renderers.UnreadableValue, renderers.NotApplicableValue, renderers.UnparsableValue)else proc_name
                create_time = '-' if type(create_time) in self.VolatilityNoneType else create_time
                local_address = '-' if type(local_address) in self.VolatilityNoneType else local_address
                remote_address = '-' if type(remote_address) in self.VolatilityNoneType else remote_address
    ...
    def update_properties(self):
        ...
        clonetext = self.context.clone()
        my_tasks = list(tasks.PsList.list_processes(clonetext, self.config['primary'], self.config['nt_symbols']))
    ...
    def get_imports(self, task, pid, context):
        '''
        ...
        global process_imports
        if not has_pefile:
            vollog.log(constants.LOGLEVEL_VVV, "pe imports need pefile module to parse the file, please install pe file and run this plugin again")
            return
        try:
            my_module = next(task.load_order_modules())
            proc_layer_name = task.add_process_layer()
        ...
        except (StopIteration, exceptions.InvalidAddressException):
            process_imports[int(pid)] = []
    ...
    def svc_scan(self):
        ...
        svscan_conf = self.context.clone()
        svscan_plug = svcscan.SvcScan(svscan_conf, self.config_path)

        # Get all services from svcscan._generator()
        svc_calc = svscan_plug._generator()
        ...
        for none, (offset, order, c_pid, start, state, c_type, name, display, binary) in svc_calc:
            name = '-' if type(name) in self.VolatilityNoneType else str(name)
            c_pid = '-' if type(c_pid) in self.VolatilityNoneType else int(c_pid)
            binary = '-' if type(binary) in self.VolatilityNoneType else str(binary)
            display = '-' if type(display) in self.VolatilityNoneType else str(display)


            # Init the service_dict[pid] = list (if there is not list in there)
            if c_pid not in service_dict:
                service_dict[c_pid] = []
            ...
            if not (offset, order, start, c_pid, name, display, c_type, state, binary) in service_dict[c_pid]:
                service_dict[c_pid].append((int(offset), int(order), str(start), c_pid, str(name), str(display), str(c_type), str(state), binary))
    ...
    def mft_parser(self):
        '''
        # Volatility 3 dont support mft_parser yet, TOADD
        ...
        with lock:
            file_path = urllib.request.url2pathname(self.context.config['automagic.LayerStacker.single_location'][file_slice:])
        command_line = r'"{}" "{}" -f "{}" windows.volexp.MftParserGui --GET-DICT "{}"'.format(sys.executable,
                                                                                            self._vol_path, file_path,
                                                                                            file_name)
        ...
        try:
            ...
            try:
                ...
                print("[-] Volatility don't fully support this version (mft parser failed)")
                return
    ...
    def file_scan(self):
        ...
        with lock:
            file_path = urllib.request.url2pathname(self.context.config['automagic.LayerStacker.single_location'][file_slice:])
        command_line = r'"{}" "{}" -f "{}" windows.volexp.FileScanGui --GET-DICT "{}"'.format(sys.executable,
                                                                                            self._vol_path, file_path,
                                                                                            file_name)
        ...
        try:
            ...
            try:
                ...
                print("[-] Volatility don't fully support this version file scan failed")
                return
    ...
    def win_obj(self):
        ...
        with lock:
            file_path = urllib.request.url2pathname(self.context.config['automagic.LayerStacker.single_location'][file_slice:])
        command_line = r'"{}" "{}" -f "{}" windows.volexp.WinObjGui --GET-DICT "{}"'.format(sys.executable, self._vol_path, file_path, file_name)
        ...
        try:
            ...
            try:
                ...
                print("[-] Volatility don't fully support this version (WinObj failed)")
                return
    ...
    def reg_hive_thread_builder(self, hive, user, context):
        '''
        ...
        # Get hive in different context
        regapi = list(hivelist.HiveList.list_hives(context = context,
        ...
        def get_all_keys(node):
            '''
            Go all over the subkeys recursive
            :param node: key
            :return: none
            '''
        ...
        try:
            get_all_keys(regapi.get_node(regapi.root_cell_offset))
        except exceptions.InvalidAddressException:
            vollog.log(constants.LOGLEVEL_VVV, "VOLEXP: Unnable to parse the hive: {} (InvalidAddressException))".format(regapi.get_name()))

        #done_run['reg_dict'] = reg_dict
        print('[+] finish build {} hive'.format(regapi.get_name()))
        ...
        reg_dict['Finish build hives'].append(int(regapi.hive_offset))
    ...
    def registry_keys(self):
        ...
        regapi_conf_dont_use_clean = self.context.clone()
        # Get volatility registry api
        reg = hivelist.HiveList.list_hives(context = regapi_conf,
                                 base_config_path = self.config_path,
                                 layer_name = self.config['primary'],
                                 symbol_table = self.config['nt_symbols'],
                                 hive_offsets = None)
        ...
        if 'Finish build hives' not in reg_dict:
            reg_dict['Finish build hives'] = []

        # Go all over the hives offset and starts a thread to find all the keys in each hive.
        for c_hive in reg:

            offset = int(c_hive.hive_offset)
            # Find hive name
            hive_path = c_hive.get_name().lower().replace('\\\\', '\\').split('\\')
            hive = hive_path[-1]

            # Find user (if this is user hive).
            user = hive_path[4] if c_hive.get_name().lower().find("\\" + "ntuser.dat") != -1 else None
            print('[+] start user {} \t hive:{}'.format(user, hive))

            # Start the thread to get all the keys (if this is not chached)
            if not int(c_hive.hive_offset) in reg_dict['Finish build hives']:
                threading.Thread(target=self.reg_hive_thread_builder,args=(offset, user, regapi_conf_dont_use_clean.clone())).start()
    ...
    def dump_reg(self):
        '''
        # This Plugin dont exists yet, just put it here in case its be there soon
        This function try to dump the registry using dumpregistry plugin
        :return: None
        '''
        threading.Thread(target=self.run_plugin_thread, args=('windows.registry.hivedump.HiveDump',)).start()
    ...
    def dump_certs(self):
        ...
        :return: None
        '''
        threading.Thread(target=self.run_plugin_thread, args=('windows.registry.certificates.Certificates',)).start()
    ...
    def run_plugin_thread(self, plugin_name):
        ...
        # Get the dump dir
        #dump_dir = urllib.request.url2pathname(volself._config.DUMP_DIR)
        ...
        command = r'"{}" "{}" -p "{}" -f "{}" {}'.format(sys.executable, self._vol_path, all_plugins[0], file_path, plugin_name)
    ...
    def run_plugin(self, plugin_name):
        ...
        plugins_path = all_plugins[0][0]

        with lock:
            # Get the file path of the dump
            file_path = urllib.request.url2pathname(volself.context.config['automagic.LayerStacker.single_location'][file_slice:])

        # Create and config the cmd like gui to run plugins using the CmdPlugin class.
        app = CmdPlugin(plugin_name, vol_path, plugins_path, file_path)
        x = root.winfo_x()
        y = root.winfo_y()
        app.geometry("+%d+%d" % (x + ABS_X, y + ABS_Y))
            ...
            for item in data:
                ...
                if "Failed to get dll name on address: " in item:
                    ...
                    if str(item).lower() in files_info:
                        ...
                        if "ProductVersion" in files_info[str(item).lower()]:
                            ProductVersion = files_info[str(item).lower()]["ProductVersion"]
                    modev_data.append((item[item.rfind('\\') + 1:], cn, Description, FileVersion, InternalName,
                                       LegalCopyright, OriginalFilename, ProductName, ProductVersion, item) +
                                      ((process_bases[4]['dlls'][item[item.rfind('\\') + 1:]],
                                        process_bases[4]['ldr'][item[item.rfind('\\') + 1:]])
                                       if item[item.rfind('\\') + 1:] in process_bases[4]['dlls'] else (-1, -1)))

            modules_table = Modules(self.NoteBook, jmp_pid_index=1, jmp_pid=4,
                                    headers=("Name", "Company Name", "Description", "File Version", "Internal Name",
                                             "Legal Copyright", "Original File Name", "Product Name", "Product Version",
                                             "Path", "Dll Base", "Ldr Address"),
                                    display=("Name", "Company Name", "Description", "Path", "Dll Base", "Ldr Address"),
    ...
    def view_comments(self, event=None):
        ...
        label = tkinter.ttk.Label(app, text="Here you can get information about all of your comments.")
        label.pack(side="top", fill="x", pady=10)
        pw = PanedWindow(app, orient='vertical')
    ...
    def update_table_all(self):
        ...
        if '/done/' not in files_info or not files_info['/done/']:
        ...
        kvo = self.get_sids_class_conf.layers[self.get_sids_class_conf.kaddr].config["kernel_virtual_offset"]
        ntkrnlmp = self.get_sids_class_conf.module(volself.config['nt_symbols'], layer_name=self.get_sids_class_conf.kaddr,
                                  offset=kvo)

        user_sids = self.get_sids_class.lookup_user_sids()
        ...
        def main_table_update(user_sids):
            ...
            for row in rows:
                ...
                e_proc = ntkrnlmp.object('_EPROCESS', e_proc - kvo)

                # Update the users information
                # Create process_security[pid] if not exists
                if int(pid) not in process_security:
                ...
                # Get the token
                try:
                    token = e_proc.Token.dereference().cast('_TOKEN')
                except exceptions.InvalidAddressException:
                    token = None
                sid_count = 0
                ...
                if token:
                    ...
                    for sid_string in token.get_sids():
                        ...
                        if sid_string in self.get_sids_class.well_known_sids:
                            sid_name = str(self.get_sids_class.well_known_sids[sid_string])
                        elif sid_string in self.get_sids_class.servicesids:
                            sid_name = str(self.get_sids_class.servicesids[sid_string])
                        elif sid_string in user_sids:
                            sid_name = str(user_sids[sid_string])
                        else:
                            sid_name_re = getsids.find_sid_re(sid_string, self.get_sids_class.well_known_sid_re)
                            if sid_name_re:
                        ...
                        for tup in process_security[int(pid)]['Groups']:
                            ...
                            attr = ntkrnlmp.object(object_type="array",
                                                   offset = token.UserAndGroups.dereference().vol.offset - kvo,
                                                   subtype = ntkrnlmp.get_type("_SID_AND_ATTRIBUTES"),
                                                   count=(token.UserAndGroupCount))[sid_count].Attributes
                            if attr > 9999:
                                sid_flag = 'Logon ID'
                            else:
                ...
                # Maybe change this way -> this string could be manipulated even from user mode.
                try:
                    path = e_proc.get_peb().ProcessParameters.ImagePathName.String
                except:
                    path = 'Invalid Peb'

                # Check if we have information of this process file (we probably get this information in the
                # self.get_proc_verinfo function)
                if str(path).lower() in files_info:

                    # Get the company name of this file
                    if "CompanyName" in files_info[str(path).lower()]:
                        cn = files_info[str(path).lower()]["CompanyName"]

                    # Get the description of this file
                    if "FileDescription" in files_info[str(path).lower()]:
                        Description = files_info[str(path).lower()]["FileDescription"]

                    # Get the version of this file
                    if "FileVersion" in files_info[str(path).lower()]:
                        version = files_info[str(path).lower()]["FileVersion"]

                    # Get the internal name for this file
                    if "InternalName" in files_info[str(path).lower()]:
                        intName = files_info[str(path).lower()]["InternalName"]

                    # Get the original file name of this file
                    if "OriginalFilename" in files_info[str(path).lower()]:
                        ofn = files_info[str(path).lower()]["OriginalFilename"]

                # Update the table (and the visual table)
                main_table.tree.item(row, values=(process, pid, ppid, cpu, pb, ws, Description, cn, dep, aslr, cfg, protection, isDebug, Prefetch, threads, handles, un, session, noh, sc, pfc, di, it, cs, winStatus, integrity, priority, ct, cycles, wsp, ppd, pwss, vs, pvs, createT, intName, ofn, wt, cl, path, cd, version, str(e_proc.vol.offset)))
                main_table.visual_drag.item(row, values=(process, pid, ppid, cpu, pb, ws, Description, cn, dep, aslr, cfg, protection, isDebug, Prefetch, threads, handles, un, session, noh, sc, pfc, di, it, cs, winStatus, integrity, priority, ct, cycles, wsp, ppd, pwss, vs, pvs, createT, intName, ofn, wt, cl, path, cd, version, str(e_proc.vol.offset)))
        ...
        if mem:
            ...
            hex_dump = ("{0}".format("\n".join(
                ["{0:#010x}  {1:<48}  {2}".format(address + o, h, ''.join(c))
                 for o, h, c in utils.Hexdump(str(mem))])))
    ...
    def save_thread(self, location_path=None):
        ...
        for key in done_run:
            ...
            if (not done_run[key]) and key in globals() and len(globals()[key]) > 0:
                ...
                """
                print(key)
                #print(process_handles[4])
                file_name = save_file_name = save_file_name or os.path.join(location_path or self._config.DUMP_DIR,r'vol3xp_{}.atz'.format(time.time()))
                try:
                    with open(file_name+key, 'wb') as handle:
                        pickle.dump(done_run[key], handle, protocol=pickle.HIGHEST_PROTOCOL)
                    with open(file_name+key, 'rb') as handle:
                        saved_data = pickle.load(handle)
                except Exception as ex:
                    print('EXCEPTT:', key)
                    print(done_run[key])
                    print(type(done_run[key]))
                    for i in done_run[key]:
                        print(type(i))
                    print('EXCEPTT:', key, ex)
                    with open(file_name+key, 'wb') as handle:
                        pickle.dump(done_run[key], handle, protocol=pickle.HIGHEST_PROTOCOL)
                    with open(file_name+key, 'rb') as handle:
                        saved_data = pickle.load(handle)
            ...
            if done_run[dict_name]:
                ...
                if dict_name == 'process_bases' and not type(done_run[dict_name][4]['proc']) is str:
                    ...
                    for pid in done_run[dict_name]:
                        ...
                        new_dic[pid]["proc"] = str(process_bases[pid]['proc'].vol.offset).strip('L')
                        new_dic[pid]["ldr"] = dict(process_bases[pid]['dlls'])
                        done_run[dict_name] = new_dic

                # Debug
                """
                with open(file_name+dict_name, 'wb') as handle:
                    pickle.dump(done_run[dict_name], handle, protocol=pickle.HIGHEST_PROTOCOL)
                with open(file_name+dict_name, 'rb') as handle:
                    savedd_data = pickle.load(handle)
                """

                saved_data[dict_name] = done_run[dict_name]

        #print('saved_data - process_dlls', len(saved_data[process_dlls]))
        #print(saved_data)
        ...
        saved_data['process_dlls'] = process_dlls
    ...
    def load(self, file_path):
        ...
        self._config._vol_path = vol_path
        self._config.API_KEY = api_key
        self._config.SAVED_FILE = save_file_name
        self._config.DUMP_DIR = dump_dir
        ...
        for task_data in process_tree_data:
            ...
            process_bases[int(pid)]['proc'] = self.ntkrnlmp.object('_EPROCESS', int(e_proc) - self.kvo) if e_proc else ''

            # Make sure we have all the process layers (for struct analyzer to use later).
            if process_bases[int(pid)]['proc']:
    ...
    def _generator(self):
        ...
        for proc_T in self.return_pstree():
            ...
            e_proc = proc_T[0]
            pid = int(e_proc.UniqueProcessId)
            ppid = int(e_proc.InheritedFromUniqueProcessId)
            try:
                proc_token = e_proc.Token.dereference().cast('_TOKEN')
            except exceptions.InvalidAddressException:
                vollog.log(constants.LOGLEVEL_VVV, "VOLEXP: Invalid Token for process {}".format(pid))
                proc_token = None
            ...
            if int(e_proc.UniqueProcessId) not in process_security:
                process_security[int(e_proc.UniqueProcessId)] = {}

            # Create a list inside the process_security[pid]['Privs']
            if 'Privs' not in process_security[int(e_proc.UniqueProcessId)]:
            ...
            for thread in e_proc.ThreadListHead.to_list("{}{}_ETHREAD".format(e_proc.get_symbol_table_name(), constants.BANG), "ThreadListEntry"):
                try:
                    thread_flag = thread.get_cross_thread_flags()
                    process_threads[int(e_proc.UniqueProcessId)][int(thread.Cid.UniqueThread)] = (int(thread.Cid.UniqueThread),
                                                                                                  hex(thread.StartAddress) if isinstance(thread.StartAddress, int) or isinstance(thread.StartAddress, int) else str(thread.StartAddress),
                                                                                                  str(thread_flag),
                                                                                                  hex(thread.Tcb.StackBase) if isinstance(thread.Tcb.StackBase, int) or isinstance(thread.Tcb.StackBase, int) else str(thread.Tcb.StackBase),
                                                                                                  int(thread.Tcb.BasePriority),
                                                                                                  int(thread.Tcb.Priority),
                                                                                                  int(thread.Tcb.UserTime),
                                                                                                  int(thread.Tcb.KernelTime),
                                                                                                  str(renderers.conversion.wintime_to_datetime(thread.CreateTime.QuadPart)),
                                                                                                  str(thread.vol.offset))
                except Exception as ex:
                    try:
                        print('[-] thread Exception:', thread.Cid.UniqueThread)
                    except:
                        pass
            ...
            try:
                handles = e_proc.get_handle_count()#ObjectTable.HandleCount if e_proc.ObjectTable.HandleCount else -1 if int(pid) not in process_handles else len(process_handles[int(pid)])
            except:
                handles = 0

            # add token info
            token_user = "{}\{}".format(str(get_right_member(proc_token, ['LogonSession.dereference.AuthorityName.get_string'])), str(get_right_member(proc_token, ['LogonSession.dereference.AccountName.get_string'])))
            token_session = str(get_right_member(proc_token, ['LogonSession.dereference.LogonId.LowPart']))
            token_session_id = str(get_right_member(proc_token, ['SessionId']))
            token_elevated = ""
            token_virtualized = ""
            token_protected = ""
            process_token[int(pid)] = (str(proc_token), token_user, token_session, token_session_id, token_elevated, token_virtualized, token_protected)
            #
            ...
            if int(pid) not in process_security:
                process_security[int(pid)] = {}
            if 'Groups' not in process_security[int(pid)]:
            ...
            if proc_token:
                first_sid_name = True
                proc_token_sid_array = self.ntkrnlmp.object(object_type="array",
                                           offset=proc_token.UserAndGroups.dereference().vol.get("offset") - self.kvo,
                                           subtype=self.ntkrnlmp.get_type("_SID_AND_ATTRIBUTES"),
                                           count=(proc_token.UserAndGroupCount))
                sid_count = 0
                ...
                for value, present, enabled, default in proc_token.privileges():
                    ...
                    try:
                        name, desc = self.privilege_info[int(value)]
                    except KeyError:
                        continue
            ...
            # Check debug. (from version 5.1 ->..)
            if self.os_version['Major'] + self.os_version['Minor'] > 5:
                isDebug = "Debuged" if e_proc.Flags & 0x2 else ""#e_proc.Flags.NoDebugInherit

            # Version 6.3 -> 1703
            if (self.os_version['Major'] == 6 and self.os_version['Minor'] == 3) or (self.os_version['Major'] == 10 and self.os_version['Build'] < 1709):
                cfg = "Enable" if e_proc.Flags & 0x00000010 else "Disable"# ControlFlowGuardEnabled
            elif self.os_version['Major'] == 10 and self.os_version['Build'] > 1709:
            ...
            if self.os_version['Major'] > 5 and e_proc.CrossSessionCreate == 1 and e_proc.WriteWatch == 1: # OverrideAddressSpace ?
            ...
            if proc_token and (self.os_version['Major'] > 5 and  e_proc.Job !=0 and proc_token.TokenFlags & TOKEN_LOWBOX or any("S-1-15-2-" in sid for sid in proc_token.get_sids())):#find immersive process #
            ...
            if (self.os_version['Major'] == 10 or (self.os_version['Major'] == 6  and self.os_version['Minor'] == 3)) and e_proc.Protection.Type > 0: #e_proc.Protection.Type==1: PsProtectionSingUntyMalwareLight, if 2 then is stronget and if 0 then no protection:###e_proc.Flag2&0x800 6.0-6.1#find protected process # _proc.Protection.Type==1: PsProtectionSingUntyMalwareLight, if 2 then is stronget and if 0 then no protection ###e_proc.Flag2&0x800 6.0-6.1and hasattr(e_proc, "Protection")
            ...
            process_performance[int(pid)] =(int(e_proc.Pcb.BasePriority), int(e_proc.Pcb.KernelTime), int(e_proc.Pcb.UserTime), int(e_proc.Pcb.KernelTime + e_proc.Pcb.UserTime), int(get_right_member(e_proc, ['Pcb.CycleTime']) or -1),#cpu
                                            ...
                                            int(handles) if type(handles) != UnreadableValue else 0, int(get_right_member(e_proc, ['ObjectTable.HandleCountHighWatermark']) or -1), 'not supported', 'not supported')#e_proc.ObjectTable.HandleCountHighWatermark
            # Get all system drivers.
            if pid == 4:
                modlist = modules.Modules.list_modules(self.context, self.config['primary'], self.config['nt_symbols'])
                for mod in modlist:
                    try:
                        mod_name = str(mod.BaseDllName.String)
                    except exceptions.InvalidAddressException:
                        mod_name = 'Failed to get device name on address: {}'.format(mod.DllBase)
                    try:
                        mod_path = str(mod.FullDllName.String)
                    except exceptions.InvalidAddressException:
                        mod_path = 'Failed to get device name on address: {}'.format(mod.DllBase)

                    process_dlls[4].append(mod_path)
                    process_bases[4]["dlls"][mod_name] = int(mod.DllBase.real)
                    process_bases[4]['ldr'][mod_name] = int(mod.vol.offset)
                    pe_comments['pid'][4][mod_path] = ['Write Your Comments Here.', 'white']
            # Get all process load dlls.
            else:
                for c_dll in e_proc.load_order_modules():
                    try:
                        dll_name = str(c_dll.BaseDllName.String)
                    except exceptions.InvalidAddressException:
                        # If we dont have name and address lets continue
                        try:
                            dll_name = 'Failed to get dll name on address: {}'.format(c_dll.DllBase)
                        except exceptions.InvalidAddressException:
                            continue
                    try:
                        dll_path = str(c_dll.FullDllName.String)
                    except exceptions.InvalidAddressException:
                        # Add just the name
                        try:
                            dll_path = 'Failed to get device name on address: {}'.format(c_dll.DllBase)
                        except exceptions.InvalidAddressException:
                            dll_path = 'Failed to get dll path and address'

                    process_dlls[int(pid)].append(dll_path)
                    process_bases[int(pid)]["dlls"][dll_name] = int(c_dll.DllBase.real)
                    process_bases[int(pid)]['ldr'][dll_name] = int(c_dll.vol.offset)
                    pe_comments['pid'][int(pid)][dll_path] = ['Write Your Comments Here.', 'white']

            process = "{} {}".format(proc_T[1] * "?\?", objects.utility.array_to_string(e_proc.ImageFileName))
            # For Processes with no peb struct (like pico and minimal)
            try:
                peb = e_proc.get_peb()
                session = e_proc.get_session_id()
                cl = peb.ProcessParameters.CommandLine.String
                path = peb.ProcessParameters.ImagePathName.String
                wt = peb.ProcessParameters.WindowTitle.String
                cd = peb.ProcessParameters.CurrentDirectory.DosPath.String
                di = peb.ProcessParameters.DesktopInfo.String
                noh = peb.NumberOfHeaps if peb.NumberOfHeaps else -1
            except:
                pass

            createT = renderers.conversion.wintime_to_datetime(e_proc.CreateTime.QuadPart)
            ...
            it = 32 if e_proc.get_is_wow64() else 64
            #end from win7
    ...
    def render_ui(self, data, root_tk=None):
        ...
        for item in range(len(list_all)):

            # if process_threads.has_key(pid): (support for memtriage if some new processes created).
            if int(list_all[item][1]) not in process_threads:#memtri
                continue
            ...
            for c_thread in list(process_threads[int(list_all[item][1])].values()): #FIXME
                ...
                try:
                    ...
                    if 'PS_CROSS_THREAD_FLAGS_TERMINATED' in str(thread_obj.get_cross_thread_flags()) or int(thread_obj.ExitTime.QuadPart) > 0:
                        termi_threads += 1
                    ...
                    elif int(thread_obj.ExitTime.QuadPart) <= 0:
                        break
                except:
                    pass
        ...
        file_menu_bar.add_command(label="Save As (cache file for this specific run)", command=lambda: self.save(tkinter.filedialog.askdirectory()))
        ...
        # Insert all the well known plugins to the plugin menu item.
        well_known_plugins = ["windows.malfind.Malfind", "windows.registry.userassist.UserAssist", "windows.ssdt.SSDT"]
        for plugin in range(len(well_known_plugins)):
            well_known_plugins_menu.add_command(label='{}'.format(well_known_plugins[plugin]),
                                                command=functools.partial(self.run_plugin, well_known_plugins[plugin]))
        ...
        menuFrame = tkinter.ttk.Frame(root)
        ...
        save_button = tk.Button(menuFrame, command=lambda: self.save(tkinter.filedialog.askdirectory()), height=15, width=15, image=save_image_icon)
        save_button.pack(side=tk.LEFT)
        tkinter.ttk.Label(menuFrame, text='  |  ').pack(side=tk.LEFT)
        ToolTip(save_button, 'Save As(cache file for this specific run)')

        properties_button = tk.Button(menuFrame, command=self.properties, height=15, width=15, image=properties_image_icon)
        properties_button.pack(side=tk.LEFT)
        tkinter.ttk.Label(menuFrame, text='  |  ').pack(side=tk.LEFT)
        ToolTip(properties_button, 'Process Properties')

        search_button = tk.Button(menuFrame, command=self.treetable.control_f, height=15, width=15, image=search_image_icon)
        search_button.pack(side=tk.LEFT)
        tkinter.ttk.Label(menuFrame, text='  |  ').pack(side=tk.LEFT)
        ToolTip(search_button, 'Find Handle or Dll [Ctrl+F]')

        lower_pane_button = tk.Button(menuFrame, command=self.treetable.control_d, height=15, width=15, image=lower_pane_image_icon)
        ...
        options_button = tk.Button(menuFrame, command=self.popup_options, height=15, width=15, image=main_properties_image_icon)
        ...
        int_var = tk.IntVar()
        int_var.set(0)
        self.loading_button = tkinter.ttk.Progressbar(menuFrame, variable=int_var, orient=tk.HORIZONTAL, length=55, mode="determinate")
        ...
        about_button = tk.Button(menuFrame, command=self.about, height=15, width=15, image=smaller_image)
        about_button.pack(side=tk.LEFT)
        ToolTip(about_button, 'About')
        investigation_tip = tkinter.ttk.Label(menuFrame, text='\tGood Luck And Have Fun In Your Analysis\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t ')
        ...
        self.modules_table = Modules(self.NoteBook, jmp_pid_index=1, jmp_pid=4,
                                headers=("Name", "Company Name", "Description", "File Version", "Internal Name",
                                         "Legal Copyright", "Original File Name", "Product Name", "Product Version",
                                         "Path", "Dll Base", "Ldr Address"),
                                display=("Name", "Path", "Dll Base", "Ldr Address"),
                                data=modev_data, resize=True)
        ...
        if (len(process_security) == 0 or any('Groups' not in gnt for gnt in list(process_security.values())) or any('Searching...' in gn for gnt in list(process_security.values()) for gn in gnt['Groups'])):
        ...
        if len(process_connections) == 0: # and int(self._KUSER_SHARED_DATA.NtMajorVersion) > 5:
        ...
        while self._run_main_loop:
            ...
            if queue.empty():
                ...
                func, args = queue.get()
                vol_debug.write('{} -> {} ({})\n'.format(time.ctime(), func, args).encode())
                if len(args) > 0:
...
class StructAnalyzer(interfaces.plugins.PluginInterface):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._config.STRUCT = self.context.STRUCT if hasattr(self.context, 'STRUCT') else self.config.get('STRUCT', None)
                ...
                for task in tasks.PsList.list_processes(self.context, self.config['primary'], self.config['nt_symbols']):
                    ...
                    if int(task.UniqueProcessId) == int(config.PID):
                        ...
                        if (not config.ADDR) and ((not config.STRUCT) or config.STRUCT.lower() == "_eprocess"):
                            config.ADDR = str(task.vol.offset)
                            config.STRUCT = "_EPROCESS"
                        break

            # Start default Struct Analyzer in the kdbg struct
            elif not config.ADDR:
                config.ADDR = str(info.Info.get_kdbg_structure(self.context, self.config_path, self.config['primary'],self.config['nt_symbols']).vol.offset)
                config.PID = self.config['primary']
                if symbols.symbol_table_is_64bit(self.context, self.config["nt_symbols"]):
                    config.STRUCT = "kdbg1!_KDDEBUGGER_DATA64"
                else:
                    config.STRUCT = "kdbg1!_KDDEBUGGER_DATA32"

        elif config.PID and str(config.PID).lower() == "physical":
            print('[!] Struct Analyzer on physical layer (assuming that the native layer name is the kernel address space).')
            config.PID = self.kaddr_space = self.context.layers['memory_layer'].name#??? #FIXME to test this .#self.kaddr_space.physical_layer()
        ...
        # Add default symbol table
        if not constants.BANG in config.STRUCT:
            config.STRUCT = self.config['nt_symbols'] + constants.BANG + config.STRUCT

        self.kaddr_space = self._config.PID
        self.struct_dict = {}
    ...
    def get_requirements(cls):
        return [
            ...
            requirements.StringRequirement(name = 'STRUCT',
                                           ...
                                           default = '_EPROCESS',
                                           optional = True),
            requirements.StringRequirement(name='ADDR',
                                           description='The Address Of The Struct',
                                           default=None,
                                           optional=True),
            requirements.StringRequirement(name='PID',
                                           description='The Pid Where The Struct Lives',
                                           default=4,
                                           optional=True)]
    ...
    def parser(self, struct, t, maxim, s, c_dict, check_functions=True):
        ...
        list_members = list(struct.vol.members.keys())

        # Go all over the functions inside the class (if this struct is a class)
        if check_functions and isinstance(struct, objects.StructType):
            for func_name in dir(struct):#get_functions(struct):
                if func_name not in list_members and func_name[0] != '_' and callable(getattr(struct, func_name)):
        ...
        for m in list_members:
            ...
            if type(val) is objects.Pointer:
                try:
                    temp_val = val.dereference()
                    if type(temp_val) == objects.Void:
                        temp_val = val.real
                    val = temp_val
                except Exception:
                    pass
            # Parse array
            if type(val) is objects.Array:
                val = val[0:]

            obj_type = eval("struct.{}.vol.type_name".format(m)).split(constants.BANG)[-1]
            obj_offset = eval("struct.{}.vol.offset".format(m))
            c_dict[m] = {'|properties|': (val.vol.offset if hasattr(val, 'vol') and issubclass(type(val), (objects.StructType, objects.UnionType, objects.ClassType)) else val, obj_offset, str(obj_type))} #struct.v() "{}.{}".format(s, m)

            # check if the members has more members
            try:
                st = list(val.vol.members.keys()) # To fail here and not in inside the self.parser
    ...
    def _generator(self):
        ...
        :return: None
        '''
        if self.context.layers['memory_layer'].name == self._config.PID:
            struct = self.context.object(self._config.STRUCT, offset=int(self._config.ADDR), layer_name='memory_layer', native_layer_name=self.config['primary'])
        else:
            struct = self.context.object(self._config.STRUCT, offset=int(self._config.ADDR), layer_name=self._config.PID)
    ...
    def render_user_interface(self):
        ...
        root.title("Struct Analyzer {} ({})".format(self._config.STRUCT, self._config.ADDR))
...
class WinObjGui(interfaces.plugins.PluginInterface):
    ...
    def __init__(self, *args, **kwargs): # Fix init from vol2
        ...
        # Fix Subsection object name for windows 10
        if int(self._KUSER_SHARED_DATA.NtMajorVersion) == 10:
            WinObjExplorer.NAME_TO_TYPE['Section'] = '_SUBSECTION'

        self._config.GET_DICT = self.config.get('GET_DICT', None)
    ...
    def winobj_builder(self, winobj_calc, c_winobj_dict, path_list, info, full_path_str):
        ...
        if len(path_list) > 0:
            ...
            if value in c_winobj_dict:
                self.winobj_builder(winobj_calc, c_winobj_dict[str(value)], path_list, info, full_path_str)
            else:
                c_winobj_dict[str(value)] = {}
                self.winobj_builder(winobj_calc, c_winobj_dict[str(value)], path_list, info, full_path_str)
            ...
            if info[0] == 'Directory':
                try:
                    if full_path_str.startswith('//'):
                        return
                    winobj_calc.SaveByPath(full_path_str)
                except Exception as ex:
                    # Error -> invalid directory -> [log that some directory parse failed]
                    vollog.log(constants.LOGLEVEL_VVV, ex)
                    return
                ...
                for obj in l:
                    info = (
                        ...
                        str(obj[winobj_calc.HEADER].get_object_type(winobj_calc.type_map)),
                        obj[winobj_calc.ADDITIONAL_INFO],
                        str(obj[winobj_calc.ADDR]))
                    if str(obj[winobj_calc.NAME]) in c_winobj_dict:
                    ...
                    if str(obj[winobj_calc.NAME]) not in c_winobj_dict:
                        c_winobj_dict[str(obj[winobj_calc.NAME])] = {'|properties|': info}
                    self.winobj_builder(winobj_calc, c_winobj_dict[str(obj[winobj_calc.NAME])], new_path_list, info,
                                        full_path_str)
    ...
    def win_obj(self, winobj_calc, winobj_dict):
        ...
        winobj_calc.update_sizes()
        root_dir = winobj_calc.get_root_directory()
        winobj_calc.parse_directory(root_dir, winobj_calc.root_obj_list)
        winobj_calc.tables["/"] = (root_dir, winobj_calc.root_obj_list)
        winobj_calc.get_directory()
    ...
    def _generator(self):
        ...
        winobj_calc = winobj.WinObj(self.context, self.config_path)
            ...
            for a, i in attributes:
                ...
                if a.startswith("FILE_NAME"):
                    if hasattr(i, "ParentDirectory"):
                        full = str(mft_entry.get_full_path(i), errors='replace')
...
class FileScanGui(interfaces.plugins.PluginInterface):
    ...
    def file_scan(self, files):
        ...
        for file in files:
            try:
                file_path = file.file_name_with_device()
                if type(file_path) in (renderers.UnreadableValue, renderers.NotApplicableValue, renderers.UnparsableValue, renderers.NotAvailableValue):
                    vollog.log(constants.LOGLEVEL_VVV, 'Unnable to get file name at address {}'.format(file))
                    continue
                header = file.get_object_header()
                access = file.access_string()
                pointer_count = int(header.PointerCount)
                handle_count = int(header.HandleCount)
            except:
                vollog.log(constants.LOGLEVEL_VVV, 'Unnable to get file name at address {}'.format(file))
                continue
            if file_path:
    ...
    def _generator(self):
        ...
        '''
        global files_scan
        files = windows.filescan.FileScan.scan_files(self.context.clone(), self.config['primary'], self.config['nt_symbols'])
        self.file_scan(files)
        #files_scan[' '] = {}
        files_scan['?shimcachemem?'] = [] # Not Supported yet
        files_scan['?userassist?']   = []
        files_scan['?shimcache?']    = [] # Not Supported yet
        files_scan['?amcache?']      = [] # Not Supported yet
        ...
        if True or int(self._KUSER_SHARED_DATA.NtMajorVersion) == 10: # TOFIX when there is shimecachemem plugin
            ...
            ua = userassist.UserAssist(self.context.clone(), self.config_path)
            ua._reg_table_name = intermed.IntermediateSymbolTable.create(self.context, self._config_path, 'windows', 'registry')
            #userassist.debug.error = lambda e: sys.stderr.write('[-] The requested key could not be found in the hive(s) searched\n')
            userassist_data = [['' if type(d) in (renderers.UnreadableValue, renderers.NotApplicableValue, renderers.UnparsableValue, renderers.NotAvailableValue) else str(d) for d in data[1]] for data in ua._generator()]#[c_data[1] for c_data in ua._generator()]
...
def main():
    ...
    # Run the portable
        ...
        filtered = {k: v for k, v in list(options.items()) if v is not None}
        options.clear()
        ...
        # From Saved File
        if 'Saved File' in options and options['Saved File'].endswith('atz'):
            #sys.setrecursionlimit(100000)
            with open(options['Saved File'], 'rb') as handle:
                saved_data = pickle.load(handle)
            file_path = saved_data['location']
            dump_dir = saved_data['dump_dir']
            api_key = saved_data['api_key'] if "api_key" in saved_data else ""
            vol_path = saved_data['vol_path']
            ...
            extended_path = interfaces.configuration.path_join(plugin_config_path, 'FILE')
            ctx.config[extended_path] = file_name
            ctx.config[interfaces.configuration.path_join(plugin_config_path, 'DUMP_DIR')] = dump_dir
            #plugins.construct_plugin(ctx, automagics, plugin, base_config_path, progress_callback, False)
            errors = automagic.run(automagics, ctx, plugin, base_config_path, progress_callback=progress_callback)
            plugin_config_path = interfaces.configuration.path_join(base_config_path, plugin.__name__)

            #plugin_config_path = interfaces.configuration.path_join(base_config_path, plugin.__name__)
            my_ve = Vol3xp(ctx, plugin_config_path, progress_callback = progress_callback)

            my_ve.config.SAVED_FILE = options['Saved File']
            my_ve.config.LOCATION = my_ve.config.location = r"{}".format(file_path)
            my_ve.config.DUMP_DIR = str(dump_dir)
            my_ve.config.API_KEY = str(api_key)
            my_ve.config.VOL_PATH = str(vol_path)
        ...
        # Run new
            ...
            location = my_arguments['Memory File']#r"file://{}".format(my_arguments['Memory File'])
            dump_dir = my_arguments['Dump-Dir']
            api_key = my_arguments['Virus Total API Key']
            vol_path = my_arguments['Volatility File Path']

            file_name = location
            single_location = "file:" + request.pathname2url(file_name)#file_name
            ctx.config['automagic.LayerStacker.single_location'] = single_location
            automagics = automagic.choose_automagic(automagics, plugin)
            extended_path = interfaces.configuration.path_join(plugin_config_path, 'FILE')
            ctx.config[extended_path] = file_name
            ...
            my_ve.config.LOCATION = my_ve.config.location = location
            my_ve.config.DUMP_DIR = dump_dir
            my_ve.config.API_KEY = api_key
            my_ve.config.VOL_PATH = str(vol_path)
            my_ve.config.SAVED_FILE = ""
...
class VolExp(Vol3xp):
    pass
class structanalyzer(StructAnalyzer):
    pass
class winobjgui(WinObjGui):
    pass
class filescangui(FileScanGui):
    pass
#endregion sc End
